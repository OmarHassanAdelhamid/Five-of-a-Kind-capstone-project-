...
\hspace{1.5em} This test ensures that the system correctly processes a valid STL file and creates a new project ready for voxel slicing and visualization.

The expected result was that the system accepts the file, creates a new project, and initiates the voxel slicing process.

The actual result confirmed that the system successfully accepted the file and created the project as expected.
...
\hspace{1.5em} This test verifies that the system correctly loads a previously saved project file while preserving magnetization and material properties.

The expected result was that the system loads the project and retains all associated properties.

The actual result confirmed that the system successfully loaded the project and voxel data, and that partitions and voxelized coordinates were retrieved as expected.
...
\hspace{1.5em} This test ensures that the system rejects unsupported file types and provides appropriate user feedback.

The expected result was that the system rejects non-STL files and displays an error indicating an unsupported format.

The actual result confirmed that the system correctly rejected non-.stl files and displayed the appropriate error message.
...
\hspace{1.5em} This test ensures that the system applies user-specified voxel dimensions and slices the model accordingly.

The expected result was that the system applies the specified dimensions and generates voxels of the requested size.

The actual result confirmed that the system correctly applied the voxel dimensions and performed slicing as expected.
...
\hspace{1.5em} This test verifies that the system scales the model to the desired size before slicing while maintaining geometric integrity.

The expected result was that the model is scaled correctly based on the specified dimensions.

The actual result confirmed that the system correctly applied model scaling as expected.
...
\hspace{1.5em} This test ensures that the system correctly resolves partial voxels when surfaces intersect voxel boundaries while preserving model integrity.

The expected result was that partial voxels are accurately represented without loss of geometric detail.

The actual result confirmed that the system correctly resolved partial voxels and maintained model accuracy.
...
\hspace{1.5em} This test verifies that when a model exceeds the MAX\_DISPLAY voxel threshold, the system partitions it into manageable display sections.

The expected result was that the model is divided into partitions without exceeding the display limit.

The actual result confirmed that the system correctly partitioned the model as expected.
...
\hspace{1.5em} This test ensures that the Visualization Manager renders a 3D model with clear visualization of individual voxels and distinct boundaries.

The expected result was that the system correctly renders the voxelized model and provides appropriate status updates (e.g., loading and ready).

The actual result confirmed that the ModelViewer component successfully rendered the model and invoked the voxel rendering utilities as expected.
...
\hspace{1.5em} This test verifies that users can navigate between model partitions and access all display sections through the interface.

The expected result was that the system retrieves available partitions and allows users to select and switch between them.

The actual result confirmed that partitions were successfully retrieved, displayed, and that selection between partitions functioned as expected.
...
\hspace{1.5em} This test ensures that the system provides intuitive rotation and zoom controls for multi-perspective viewing of the model.

The expected result was that users can interact with the model smoothly through rotation and zoom operations.

The actual result confirmed that the system supports responsive and intuitive model interaction as expected.
...
\hspace{1.5em} This test verifies that users can isolate a specific layer to facilitate property assignments within the model.

The expected result was that the selected layer is isolated and displayed in the layer editor with accessible layer data.

The actual result confirmed that the system successfully isolated the selected layer and displayed it correctly within the layer editor.
...
\hspace{1.5em} This test ensures that the system provides clear visual feedback for selected voxels within a layer.

The expected result was that selected voxels are distinctly highlighted and that both click and lasso selection modes function correctly.

The actual result confirmed that voxel selection was clearly indicated and that both selection modes operated as expected.
\end{enumerate}
...
\hspace{1.5em} This test verifies that voxel colours update to reflect material assignment.

The expected result was that assigned materials are visually indicated through updated voxel colours, providing clear feedback to the user.

The actual result confirmed that material assignment was correctly reflected through voxel colour updates as expected.
...
\hspace{1.5em} This test ensures that voxels display a clear visual indication of magnetization assignment.

The expected result was that assigned magnetization is visibly represented on the voxels (e.g., through an indicator or marker).

The actual result confirmed that magnetization assignments were correctly displayed with clear visual feedback.
\end{enumerate}
...
\hspace{1.5em} This test ensures that when a valid CAD file is imported, the Visualization Manager receives complete voxel data and renders the 3D model without data loss or corruption.

The expected result was that voxel data is successfully transferred from the import process to the visualization component and rendered correctly.

The actual result confirmed that the system successfully transferred voxel data and rendered the model without data loss or corruption.
\end{enumerate}
...
\hspace{1.5em} These tests verify that the system correctly supports both individual and group magnetization assignment for voxels.

The expected result was that assigned magnetization values are correctly applied, saved, and visually reflected on the selected voxels.

The actual result confirmed that magnetization assignments were successfully applied and displayed for both individual and group selections as expected.
...
\hspace{1.5em} This test was intended to verify that users can create and reuse favourite magnetization vectors.

The expected result was that magnetization vectors could be saved and selected from a favourites list.

The test was not executed as this feature was determined to be out of scope for the final system and is no longer considered a required functionality.
...
\hspace{1.5em} These tests verify that the system supports both individual and group material assignment for voxels.

The expected result was that material IDs are correctly applied, saved, and visually reflected on the selected voxels.

The actual result confirmed that material assignments were successfully applied and displayed for both individual and group selections as expected.
...
\hspace{1.5em} This test ensures that material IDs can be associated with descriptive labels (e.g., ``Steel'' for ID 1).

The expected result was that users can assign labels to material IDs and that these labels are correctly stored and displayed.

The actual result confirmed that material labels were successfully assigned and reflected as expected.
...
\hspace{1.5em} This test was intended to verify that properties can be copied from one layer to another.

The expected result was that selected properties are replicated accurately across layers.

The test was not executed as this feature was identified as a stretch goal and was not implemented in the final system, and is therefore considered out of scope.
...
\hspace{1.5em} This test ensures that the system automatically saves changes after property assignment.

The expected result was that any changes made to voxel properties are automatically saved without requiring manual user action.

The actual result confirmed that changes were automatically saved as expected.
...
\hspace{1.5em} This test was intended to verify that users can undo and redo edits within the system.

The expected result was that users can revert and reapply changes through undo and redo functionality.
...
\hspace{1.5em} This test verifies that users can select an entire layer and apply assignments to all voxels within that layer.

The expected result was that all voxels in the selected layer are correctly identified and updated with the assigned properties.

The actual result confirmed that layer selection and assignment were successfully applied as expected.
...
\hspace{1.5em} These tests ensure that users can manually add and remove voxels within the model.

The expected result was that voxels can be added or deleted correctly, and that appropriate error handling is triggered when attempting to modify non-existent voxels.

The actual result confirmed that voxel addition and deletion operations functioned correctly and that error handling behaved as expected.
...
\hspace{1.5em} These tests were intended to verify that users can reset material and magnetization properties on selected voxels.

The expected result was that selected voxels are restored to their default material and magnetization values.
...
\hspace{1.5em} This test was intended to verify that users can manually save the project.

The expected result was that users can trigger a save action and receive confirmation that the project has been saved.

The test was not executed as manual save functionality was replaced by an auto-save mechanism in the final system, and is therefore considered out of scope.
\end{enumerate}
...
\hspace{1.5em} These tests were intended to verify that the system validates magnetization completeness before export and prevents export of incomplete models.

The expected result was that the system checks for completeness of magnetization data and provides appropriate errors or prompts when validation fails.

The test was not executed as this validation functionality is not applicable in the final system and is therefore considered out of scope.
...
\hspace{1.5em} This test verifies that the system can export a standalone file containing all voxel metadata.

The expected result was that the file is correctly generated and successfully saved by the user.

The actual result confirmed that the file was successfully exported and saved as expected.
...
\hspace{1.5em} This test ensures that users can export the project in its native format.

The expected result was that the system generates a valid project file and handles errors appropriately for non-existent or invalid projects.

The actual result confirmed that project export functioned correctly and that error handling behaved as expected.
...
\hspace{1.5em} This test was intended to verify that users receive feedback during the export process.

The expected result was that the system provides progress updates or indicators while exporting files.
...
\hspace{1.5em} This test was intended to verify that the system can export a model with an additional summary of its properties.

The expected result was that the exported file includes both voxel data and a summary of model properties.

The test was not executed as this functionality is not applicable in the final system and is therefore considered out of scope.
...
\hspace{1.5em} This test verifies that if an export operation is interrupted, the original project data remains intact.

The expected result was that no data corruption occurs and the original project state is preserved after a failed export.

The actual result confirmed that project data remained intact and was not corrupted during simulated failure conditions.

\item \textbf{test-FR-XM-8 Export Performance Validation}

\hspace{1.5em} This test ensures that the export process meets the required performance thresholds.

The expected result was that export operations complete within acceptable performance limits.

The actual result confirmed that export performance met the required thresholds as expected.
...
In particular, tests labeled \texttt{test-US} correspond to usability evaluation, tests labeled \texttt{test-PS} correspond to performance evaluation, and tests labeled \texttt{test-CO} correspond to portability testing. 
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{images/usability_chart.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{images/import_scalability.pdf}
...
The results of operation latency, measured in terms of user interaction counts rather than time, are summarized in Table~\ref{tab:operation-latency}.
\begin{table}[H]
\centering
\caption{Operation Latency Results (Measured in User Interaction Counts)}
\label{tab:operation-latency}
\begin{tabular}{|c|c|}
\hline
...
\end{tabular}
\end{table}
In this context, latency is defined as the number of user interactions required to complete a task rather than elapsed time. This approach reflects usability and efficiency, as fewer interactions indicate a more streamlined workflow. The results show that both simple and complex operations require a low and consistent number of interactions, demonstrating that the system maintains efficient user interaction regardless of task complexity.
The latency for the various operations is visualized in Figure~\ref{fig:operation_latency}.

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{images/operation_latency.pdf}
...
The results demonstrate that the system scales appropriately with increasing voxel counts while maintaining acceptable performance. Import times increase with voxel count; however, the increase is sub-linear rather than strictly linear. This indicates that the system scales efficiently as model size grows, since the rate of increase in processing time slows relative to the increase in voxel count. This behavior suggests that the voxelization and import pipeline is optimized and does not degrade significantly for larger models.%Import times increased gradually as model size grew, indicating that the voxelization and visualization pipeline handles larger models efficiently.
...
Portability testing was performed according to the procedures defined in the VnV plan (tests \texttt{test-CO-1} through \texttt{test-CO-4}). The evaluation focused on verifying operating system compatibility, file import and export compatibility, project file reliability during unexpected interruptions, and adherence to coding standards.

Portability testing followed the procedures defined in the VnV Plan through tests \texttt{test-CO-1} to \texttt{test-CO-4}, covering operating system compatibility, file compatibility, project file fail-safe behaviour, and PEP8 coding standard compliance.
...
The portability tests demonstrate that the application satisfies the compatibility and reliability requirements defined in the VnV plan. The system operates correctly across supported operating systems, maintains file integrity during unexpected interruptions, and adheres to recognized Python coding standards. These results confirm that the system meets the specified portability requirements.
...
\begin{longtable}{c
  ...
  >{\raggedright\arraybackslash}p{5cm} c}
  ...
  \textbf{Expected Result} & \textbf{Assertions} & \textbf{Result} \\
  \midrule
  \endfirsthead
  ...
  TC1 & F211 & Provide a valid STL file to the system through the file settings panel & STL file is uploaded successfully to the system, and saved to the sample file directory & Assertions were made that once STL is uploaded, the system will save it to the sample file directory. & \cellcolor{green} Pass \\ \midrule
  TC2 & F211 & Browse all uploaded STL files in the sample file directory & System displays a list of all uploaded STL files in the sample file directory & Assertions were made in the backend to verify that the system displays a list of all uploaded STL files in the sample file directory. & \cellcolor{green} Pass \\ \midrule 
  TC3 & F211 & Browse all uploaded STL files, without ever uploading a STL file & System displays an empty list of uploaded STL files & Assertions were made that if the file directory is empty, the system will display an empty list. & \cellcolor{green} Pass \\ \midrule
  TC4 & F211 & Provide a file with unsupported format (e.g.\ .txt, .jpg, .pdf) to system through file settings panel & System rejects the file and displays an error indicating unsupported format & Assertions were made in the frontend that once user attempts to upload a file with an unsupported format, the system will display an error indicating the file format is unsupported. & \cellcolor{green} Pass \\ \midrule
  TC5 & F212 & Open a previously uploaded STL file & System loads the STL file from the sample file directory & Assertions were made in the backend to verify that the system loads the STL file from the sample file directory. & \cellcolor{green} Pass \\
\end{longtable}
...
\begin{longtable}{c
  ...
  TC6 & F213 & Specify custom voxel size & System applies the dimensions and slices the model into voxels of the requested size & Assertions were done to verify the voxelized model has the same voxel size as the specified voxel size  & \cellcolor{green} Pass \\ \midrule
  TC7 & F214 & Specify custom geometric model size & System scales the model to desired model size before slicing & Assertions were made to verify the scale factor of the final mesh is equal to the specified geometric model size multiplied by the original model& \cellcolor{green} Pass \\ \midrule
  TC8 & F216 & Import a model containing more than MAX\_VOXELS voxels. & System partitions the model into display sections not exceeding MAX\_VOXELS & Assertions were made to verify that the number of partitions that are made which should equal 8 for a model with MAX\_VOXELS voxels& \cellcolor{green} Pass \\ \midrule
  TC9 & F216 & Import a model containing less than MAX\_VOXELS voxels & System returns only one partition with all voxels visible & Assertions were made to verify that the number of partitions that are made which should equal 1 for a model with less than MAX\_VOXELS voxels & \cellcolor{green} Pass \\ \midrule
  TC10 & F216 & Delete a partition with no voxels & System deletes the partition and returns the model to the previous state & Asserts partition count drops from 8 to 4 after empty partitions are removed; asserts all original voxels remain intact across the surviving partitions. & \cellcolor{green} Pass \\ \midrule
  TC11 & F213, F214, F215 & Voxelize uploaded STL model into voxels& System voxelizes the model into voxels of the requested size using Triangle Meshes & Asserts verify that each stage of the voxelization pipeline, mesh creation, voxelization, coordinate extraction, and database initialization, all executed exactly once, and that the resulting project contains the correct number of voxels. & \cellcolor{green} Pass \\ \midrule
  TC12 & F213, F214, F215 & Voxelize non-existing STL model into voxels& System returns an error indicating the STL model does not exist & Asserts are done to ensure system returns a 404 error code in case of non-existing file, and message saying "File.stl not found on server" & \cellcolor{green} Pass \\
\end{longtable}
...
\begin{longtable}{c
  ...
  TC13 & F221 & Import a STL model into the system & System renders a non-voxelized 3D STL model using Three.js & Asserts verify the status changes, starting with \texttt{`loading'} and then \texttt{`ready'} when a model is provided. & \cellcolor{green} Pass \\ \midrule
  TC14 & F221 & Open a voxelized project from file directory & System renders a voxelized 3D model using Three.js broken down into layers & Asserts in frontend verify that opening a voxelized project triggers partition retrieval, loads the selected partition's voxel data, and renders the 3D model when coordinates are available. & \cellcolor{green} Pass \\ \midrule
  TC15 & F222 & Open a voxelized project from the file directory where the number of voxels is greater than MAX\_VOXELS & System displays a partitions panel with more than 1 partition & Asserts in frontend verify that partitions are retrieved for the selected project and that multiple partition names are displayed in the partitions panel. & \cellcolor{green} Pass \\ \midrule
  TC16 & F222 & Open a voxelized project from the file directory where the number of voxels is less than MAX\_VOXELS & System displays a partitions panel with only 1 partition &Asserts in frontend show that no partitions are returned from the backend and ``No partitions found" message shown. & \cellcolor{green} Pass \\
...
\begin{longtable}{c
  ...
  TC17 & F222 & Pick a specific partition to focus on from the partitions panel. & The selected partition is isolated and displayed as a 3D view without any other partitions. & Assertions made to verify that when selecting a partition from the list of partitions, the correct partition is rendered. & \cellcolor{green} Pass \\ \midrule
  TC18 & F223 & Rotate the model using interface controls. & Model rotates smoothly and intuitively & Asserts verify that the camera is updated correctly in response to a window resize event, confirming the rendering pipeline adjusts to viewport changes. & \cellcolor{green} Pass \\ \midrule
  TC19 & F223 & Zoom the model using interface controls. & Model zooms smoothly and intuitively & Asserts verify that the model zooms in when the `+' key is pressed and zooms back out when the `-' key is pressed. & \cellcolor{green} Pass \\ \midrule
  TC20 & F224 & Select a specific layer to focus on from the 3D model display. & Selected layer is isolated, layer editor is opened displaying the layer voxels and properties in a 2D view. & Asserts verify that selecting a layer fetches the correct layer data and opens the layer editor displaying the selected layer. & \cellcolor{green} Pass \\ \midrule
  TC21 & F225 & Select individual voxel within the focused layer from the layer editor. & The selected voxel is highlighted in the 2D layer view & Asserts verify that after selecting a voxel, the selection properties are not null and the voxel's material is correctly reported. & \cellcolor{green} Pass \\ \midrule
  TC22 & F225 & Select multiple voxels within the focused layer from the layer editor using lasso selection. & The selected voxels are highlighted in the 2D layer view & Asserts verify that clicking the Lasso button activates lasso selection mode in the layer editor. & \cellcolor{green} Pass \\ \midrule
  TC23 & F225 & Select all voxels in the partition on the 3D view (CTRL+A) & All voxels in the partition are highlighted in the 3D view & Asserts verify that pressing Ctrl+A triggers the select all action in the 3D view exactly once. & \cellcolor{green} Pass \\ \midrule
  TC24 & F225 & Select all voxels in the layer on the 2D layer view (CTRL+A) & All voxels in the layer are highlighted in the 2D layer view & Asserts verify that pressing Ctrl+A in the 2D layer view selects all voxels and their properties are correctly accessible. & \cellcolor{green} Pass \\
\end{longtable}
...
\begin{longtable}{c
  ...
  TC25 & F226 & Assign material IDs to a voxel or multiple voxels in the layer editor. & The color of the voxels is updated to the assigned material color. & Asserts verify that assigning a material ID to selected voxels sends the correct update request to the backend with the new material and target partition. & \cellcolor{green} Pass \\ \midrule
  TC26 & F227 & Assign magnetization to a voxel or multiple voxels in the layer editor. & The voxels assigned will be highlighted with an X icon indicating magnetization. & Asserts verify that assigning a magnetization vector to selected voxels sends the correct update request to the backend with the new magnetization and target partition. & \cellcolor{green} Pass \\

\end{longtable}

\subsection{Editing Manager Tests}
The Editing Manager tests are organized into three subsections: editing a voxel (TC27--TC48), utilizing the history manager (TC49--TC57), and saving project (TC58--TC61).

\subsubsection{Editing a voxel}
Table~\ref{table:em_voxel_tests} gives the results for voxel editing tests (TC27--TC48): magnetization and material assignment, layer access, and adding, deleting, or resetting voxels.

\begin{longtable}{c
    >{\raggedright\arraybackslash}p{1.5cm}
    >{\raggedright\arraybackslash}p{3cm}
    >{\raggedright\arraybackslash}p{3cm}
  >{\raggedright\arraybackslash}p{4.5cm} c}
  ...
  TC27 & F231 & Update magnetization of a single voxel (e.g.\ x=1.0, y=0.0, z=0.0). & System saves the magnetization vector of the voxel to the database & Asserts verify that the target voxel's magnetization is updated correctly in the database while all other voxels remain unchanged. & \cellcolor{green} Pass \\ \midrule
  TC28 & F231 & Update magnetization of multiple voxels (e.g.\ x=1.0, y=0.0, z=0.0). & System saves the magnetization vector of the voxels to the database & Asserts verify that all targeted voxels have their magnetization updated correctly in the database while all other voxels remain unchanged. & \cellcolor{green} Pass \\ \midrule
  TC29 & F231 & Update magnetization of a non-existing voxel (e.g.\ x=1.0, y=0.0, z=0.0). & System returns an error indicating the voxel does not exist & Asserts verify that attempting to update a non-existent voxel does not create any new entry in the database and leaves all existing voxels unaffected. & \cellcolor{green} Pass \\ \midrule
  TC30 & F231 & Update magnetization of a voxel with invalid magnetization vector (e.g.\ x=1.0, y=0.0, z=0.0). & System returns an error indicating the magnetization vector is invalid & Asserts verify that an error is raised when the polar angle is outside the valid range (0--180) or the azimuth angle is outside the valid range (0--360). & \cellcolor{green} Pass \\ \midrule
  TC31 & F233 & Assign material ID to a single voxel (e.g.\ 2). & System saves the material ID to the voxel and in the database & Asserts verify that the target voxel's material is updated correctly in the database, the change is recorded in the history, and a success response is returned. & \cellcolor{green} Pass \\ \midrule
  TC32 & F233 & Assign material ID to multiple voxels (e.g.\ 2). & System saves the material ID to the voxels and in the database & Asserts verify that all targeted voxels have their material updated correctly in the database, the change is recorded in the history, and a success response is returned. & \cellcolor{green} Pass \\ \midrule
  TC33 & F233 & Assign material ID to a non-existing voxel (e.g.\ 2). & System returns an error indicating the voxel does not exist & Asserts verify that attempting to assign a material to a non-existent voxel does not modify the database. & \cellcolor{green} Pass \\ \midrule 
  TC34 & F231, F233 & Assign material ID to a voxel and magnetization to a voxel (e.g.\ 2, x=1.0, y=0.0, z=0.0). & System saves the material ID and magnetization vector to the voxel and in the database & Asserts verify that material and magnetization are saved correctly when assigned together, and that providing both simultaneously in a single update request is rejected with an error. & \cellcolor{green} Pass \\ \midrule
  TC35 & F234 & Add label ``Steel'' to material IDs. & The label ``Steel'' is added to the next available material ID in the database. & Asserts verify that adding a label ``Steel'' stores the association in the database and asserts that retrieving the label's associated material ID returns ``Steel'' & \cellcolor{green} Pass \\ \midrule
  TC36 & F235 & Copy assigned properties from one layer to another layer. & Target layer voxels receive the same material and magnetization properties as the source. & Asserts verify that pasting copied properties onto a layer sends the correct material update to the backend for the target partition. & \cellcolor{green} Pass \\ \midrule 
  TC37 & F238 & Get the list of layers in the project. & The list of layers is returned. & Asserts verify that the layer list response contains the correct number of layers and their corresponding index--coordinate pairs for each axis. & \cellcolor{green} Pass \\ \midrule
  TC38 & F238 & Get a specific layer in the project based on the layer index and axis. & The specific layer is returned. & Asserts verify that the correct set of voxels is returned for the requested layer index and axis, and that the response includes the correct voxel count. & \cellcolor{green} Pass \\ \midrule
  TC39 & F238 & Get specific layer with non-existing layer index. & System returns an error indicating the layer index does not exist. & Asserts verify that requesting a layer at a non-existent index returns a 404 error and an empty voxel list. & \cellcolor{green} Pass \\ \midrule
  TC40 & F238 & Get specific layer with non-existing axis. & System returns an error indicating the axis does not exist. & Asserts verify that a request with an invalid axis value is rejected with a validation error before reaching the service layer. & \cellcolor{green} Pass \\ \midrule
  TC41 & F238 & Get specific layer with non-existing project. & System returns an error indicating the project does not exist. & Asserts verify that requesting a layer from a non-existent project returns a 404 error. & \cellcolor{green} Pass \\ \midrule
  TC42 & F238 & Get specific layer with non-existing partition. & System returns an error indicating the partition does not exist. & Asserts verify that requesting a layer from a non-existent partition returns a 404 error. & \cellcolor{green} Pass \\ \midrule
  TC43 & F239 & Add a new voxel at new indices. & A new voxel is added to the database at the new indices with default material and magnetization. & Asserts verify that newly added voxels appear in the database with default material and magnetization, and that the addition is recorded in the history. & \cellcolor{green} Pass \\ \midrule
  TC44 & F239 & Delete an existing voxel. & A voxel is removed from the database at the existing indices. & Asserts verify that deleted voxels are removed from the database and the deletion is recorded in the history. & \cellcolor{green} Pass \\ \midrule
  TC45 & F239 & Delete a non-existent voxel. & System returns an error indicating the voxel does not exist. & Asserts verify that attempting to delete a non-existent voxel leaves the database unchanged. & \cellcolor{green} Pass \\ \midrule
  TC46 & F2310 & Reset materials on selected voxels. & The voxels are reset to the default material in the database. & Asserts verify that selected voxels have their material reset to the default value, magnetization remains unchanged, and the reset is recorded in the history. & \cellcolor{green} Pass \\ \midrule
  TC47 & F2310 & Reset magnetizations on selected voxels. & The voxels are reset to the default magnetization in the database. & Asserts verify that selected voxels have their magnetization reset to the default value in the database, and the reset is recorded in the history. & \cellcolor{green} Pass \\ \midrule
  TC48 & F2310 & Reset materials and magnetizations on selected voxels. & The voxels are reset to the default material and magnetization in the database. & Asserts verify that both material and magnetization are simultaneously reset to their default values for all selected voxels. & \cellcolor{green} Pass \\
\end{longtable}

\subsubsection{Utilizing the history manager}
Table~\ref{table:em_history_tests} gives the results for history manager tests (TC49--TC57): undo, redo, and clear history.
...
\begin{longtable}{c
  ...
  TC49 & F237 & Undo any updates made to a voxel. & The changes are undone and the voxel returns to the previous state. & Asserts verify that undoing a voxel update restores the previous state in the database, moves the change to the redo stack, and returns a success response. & \cellcolor{green} Pass \\ \midrule
  TC50 & F237 & Redo any undone updates to a voxel. & The changes are redone and the voxel returns to the previous state. & Asserts verify that redoing an undone update reapplies the change to the database, clears the redo stack, and returns a success response. & \cellcolor{green} Pass \\ \midrule
  TC51 & F237 & Undo any added voxels. & The voxels are removed from the database. & Asserts verify that undoing an add operation removes the previously added voxels from the database and returns a success response. & \cellcolor{green} Pass \\ \midrule
  TC52 & F237 & Redo any deleted voxels. & The voxels are added back to the database. & Asserts verify that redoing a delete operation restores the previously removed voxels to the database and returns a success response. & \cellcolor{green} Pass \\ \midrule
  TC53 & F237 & Undo any reset material on a voxel. & The voxel is reset to the default material. & Asserts verify that undoing a reset material operation restores the original voxel data and returns a success response. & \cellcolor{green} Pass \\ \midrule
  TC54 & F237 & Redo any reset material on a voxel. & The voxel is reset to the default material. & Asserts verify that redoing a reset material operation re-applies the reset and returns a success response. & \cellcolor{green} Pass \\ \midrule
  TC55 & F237 & Undo when the history is empty. & System returns an error indicating the history is empty & Asserts verify that attempting to undo with an empty history raises an error and returns a 400 response. & \cellcolor{green} Pass \\ \midrule
  TC56 & F237 & Redo when the redo stack is empty. & System returns an error indicating the redo stack is empty & Asserts verify that attempting to redo with an empty redo stack raises an error and returns a 400 response. & \cellcolor{green} Pass \\ \midrule
  TC57 & F237 & Clear history. & Both history and redo stacks are empty. & Asserts verify that clearing the history empties both the undo and redo stacks, and that recording a new change after an undo also clears the redo stack. & \cellcolor{green} Pass \\
\end{longtable}

\subsubsection{Saving project}
Table~\ref{table:em_save_tests} gives the results for saving project tests (TC58--TC61).
...
\begin{longtable}{c
  ...
  TC58 & F236 & Save the project automatically. & The project is saved to the database. & Asserts verify that the project is saved to the database when the user makes changes and the auto-save feature is enabled. & \cellcolor{green} Pass \\ \midrule
  TC59 & F2311 & Save the project manually. & The project is saved to the database. & Asserts verify that saving a project triggers a file download and confirms the filename in a success message. & \cellcolor{green} Pass \\ \midrule
  TC60 & F2311 & Save a non-existent project. & System returns an error indicating the project does not exist. & Asserts verify that attempting to save without a loaded project displays an appropriate error message via both the menu and the keyboard shortcut. & \cellcolor{green} Pass \\ \midrule
  TC61 & F2311 & Save a project with invalid data. & System returns an error indicating the project data is invalid. & Asserts verify that a failed save operation displays an error message to the user. & \cellcolor{green} Pass \\
\end{longtable}

\subsection{Export Manager Tests}

\subsubsection{Export Validation and File Export}
Table~\ref{table:xm_tests} gives the results for export validation and file export (TC62--TC70).
...
\begin{longtable}{c
  ...
  TC62 & F241 & Validate all voxels have magnetization. & System checks if all voxels have magnetization and returns an error if any voxels are missing magnetization. & Asserts verify that validation fails when any voxel has both polar and azimuth magnetization angles at their default (unset) value of zero, whether for a single voxel or all voxels in the project. & \cellcolor{green} Pass \\ \midrule
  TC63 & F241 & Validate all voxels have material. & System checks if all voxels have material and returns an error if any voxels are missing material. & Asserts verify that validation fails when any voxel has a material ID of zero (unassigned), whether for a single voxel or all voxels in the project, and that validation passes when all voxels have valid materials and magnetization. & \cellcolor{green} Pass \\ \midrule
  TC64 & F242 & Export CSV file of an existing project. & System exports a CSV file with all voxel metadata. & Asserts verify that the exported CSV contains the correct column headers and voxel data rows, is returned as a downloadable attachment with the correct filename, and the export function completes successfully. & \cellcolor{green} Pass \\ \midrule
  TC65 & F242 & Export CSV file of a non-existent project. & System returns an error indicating the project does not exist. & Asserts verify that attempting to export a non-existent project returns a 404 error. & \cellcolor{green} Pass \\ \midrule
  TC66 & F242 & Export CSV file of a project with invalid data. & System returns an error indicating the project data is invalid. & Asserts verify that exporting a project with incomplete voxel data returns a 400 error, and that the export function returns false for an empty project directory. & \cellcolor{green} Pass \\ \midrule
  TC67 & F243 & Export project file of an existing project. & System exports a project file with all project data. & Asserts verify that the project file is exported successfully with the correct column headers, data rows, and downloadable attachment headers. & \cellcolor{green} Pass \\ \midrule
  TC68 & F243 & Export project file of a non-existent project. & System returns an error indicating the project does not exist. & Asserts verify that attempting to export a non-existent project file returns a 404 error. & \cellcolor{green} Pass \\ \midrule
  TC69 & F243 & Export project file of a project with invalid data. & System returns an error indicating the project data is invalid. & Asserts verify that a database error during export returns a 500 error response. & \cellcolor{green} Pass \\ \midrule
  TC70 & F241 & Export operation is interrupted (simulated failure). & The original project data remains intact and unaltered. & Asserts verify that when an export operation fails, any temporary files are cleaned up and the original project data remains intact. & \cellcolor{green} Pass \\
\end{longtable}
...
\begin{itemize}
  ...
  \item \textbf{Favourite Magnetization Vectors:} Add a favourite magnetization vectors feature to the system to allow users to save and reuse frequently used magnetization vectors.
...
TC31 & \obox & \obox & \xbox & \obox & \obox & \obox \\
TC32 & \obox & \obox & \xbox & \obox & \obox & \obox \\
TC33 & \obox & \obox & \xbox & \obox & \obox & \obox \\
TC34 & \xbox & \obox & \xbox & \obox & \obox & \obox \\
TC35 & \obox & \obox & \obox & \xbox & \obox & \obox \\
TC36 & \obox & \obox & \obox & \obox & \xbox & \obox \\
TC37 & \obox & \obox & \obox & \obox & \obox & \xbox \\
...
TC66 & \xbox & \xbox & \obox \\
TC67 & \xbox & \xbox & \obox \\
CO-3 (Portability Test) & \xbox & \obox & \obox \\
...
TC31 & \xbox & \xbox & \obox & \obox & \obox & \obox & \xbox \\
TC32 & \xbox & \xbox & \obox & \obox & \obox & \obox & \xbox \\
TC33 & \xbox & \xbox & \obox & \obox & \obox & \obox & \xbox \\
TC34 & \xbox & \xbox & \obox & \obox & \obox & \obox & \xbox \\
TC35 & \xbox & \xbox & \obox & \obox & \xbox & \obox & \obox \\
TC36 & \xbox & \xbox & \xbox & \xbox & \obox & \obox & \xbox \\
TC37 & \xbox & \xbox & \xbox & \obox & \obox & \obox & \obox \\
...
\bigskip\bigskip
\textbf{Omar: } The main thing that went well was connecting the VnV plan with the VnV report. The plan already defined the tests clearly, so it was straightforward to structure the report around usability, performance, portability, maintainability, and security. The usability testing also went well since we were able to get feedback from both technical and non-technical users, which helped validate that the interface and main features were intuitive.
\newline
\newline
