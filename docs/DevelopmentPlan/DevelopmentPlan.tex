...
\begin{enumerate}
  ...
  \item \textbf{Demonstrate that each voxel/group of voxels can be magnetized.} This also shows that we are capable of rendering the converted model in 3D.
...
\begin{enumerate}
  \item \textbf{Rendering the model in 3D may be slow.} Initial iterations may be laggy due to lack of group experience with 3D rendering. Should this be the case, the group will need to revisit optimization strategies to find ways of reducing lag.
  \item \textbf{File converting time is longer than converting the model manually.} Whilst stakeholders have stated that the process simply being automated is the ultimate goal, the fact remains that our software should be reasonably more efficient than painstakingly re-building the model by hand. If this should occur, our group will need to re-analyse how the files are converted from the CAD file into our software and investigate if steps can be taken to the initial files to speed up the process, or if scope regarding voxel customisability should be paired back for initial iterations.
\end{enumerate}
