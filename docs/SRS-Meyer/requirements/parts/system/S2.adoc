...
. [[NF213]] *Ease of use:* Import Manager shall ensure projects can be created/opened in less than MAX_INTERACTIONS. (NF213)
...
. [[NF223]] *Accessible colours:* Visualization Manager shall ensure all colours utilized for all interfaces (e.g. voxel highlight colours, text colours, background colours) have sufficient contrast in accordance with WCAG 2.0 guidelines and can be distinctively recognized under standard lighting conditions. (NF223)
...
. [[NF231]] *Ease of use:* Editing Manager shall ensure that all interactions covered in the above requirements (e.g. assigning properties, layer selection, undo/redo) can be completed in MAX_INTERACTIONS or less by a user who is familiar with how the printer operates, creating an intuitive interface. (NF231)
...
. [[NF243]] *Ease of use:* Export Manager shall ensure projects can be exported and/or saved in under MAX_INTERACTIONS. (NF243)
...
. [[GEN1]] *Manual Accessibility:* The system shall include a user's manual that is details primary use cases, minimizes description to what is necessary, and considered interpretable by primary users. (GEN1)
...
. [[GEN2]] *Implementation Maintainability:* The system shall be implemented in a modular, maintainable, and extensible fashion. (GEN2)
