...
April 6, 2026 & All & Final Revisions\\
\bottomrule
\end{tabularx}
...
\noindent This hazard analysis examines the potential risks and failures that may arise during the development and operation 
of the voxel-based design platform used for automating multi-material 3D printing. The analysis covers all major stages 
of the system’s lifecycle, including CAD model import, voxelization, visualization, editing, and export. Each of these 
stages introduces possible hazards such as file corruption, rendering issues, data loss, or miscommunication between the 
user interface and internal processes. The scope also considers hazards resulting from external factors such as hardware 
limitations, printer software compatibility, and computational resource constraints. The analysis is limited to hazards 
directly related to the software system and its interactions, and excludes physical hardware malfunctions or the user misuse 
outside normal operating conditions. See the list below for a high-level overview of hazard classes within the scope of this analysis.\\

\begin{itemize}
    \item \textbf{File processing failures:} Includes import and export issues, incorrect saving or lack of saves that could lead to progress loss or errors in printing.
    \item \textbf{Lack of user protections:} Includes not protecting users from inputting incorrect form data, or otherwise actions that allow users to perform actions that result in either data loss or unbeneficial model edits.
    \item \textbf{Visualization failures:} Includes rendering issues that lead to user misinput or lack of rendering entirely leading to delays in model iteration and development.
\end{itemize}

\noindent The purpose of this hazard analysis is to identify, evaluate, and mitigate risks that could compromise the reliability, 
safety, or effectiveness of the voxel design platform. Conducting this analysis helps the development team anticipate 
failures early in the project lifecycle, reduce potential losses of time, data, and materials, and improve the system’s 
overall dependability. The findings from this process will inform safety requirements, guide testing and validation efforts, 
and contribute to the creation of a robust, efficient, and trustworthy tool for research and development in multi-material 
3D printing.
...
In this section, we will be analyzing the failure modes and effects of the system. \autoref{TblFMEApt1} \& \autoref{TblFMEApt2} will break down the potential failure modes, their causes, effects, recommended actions, and the safety requirements that are associated with them.
\begin{table}[H]
\centering
...
\hline
Editing Manger and Export Manager & Exported file contains incorrect magnetization vectors or material IDs. & \textbullet{} Unexpected software bug that allows user to input invalid values & \textbullet{} The custom 3D printer software cannot read the file correctly due to the unexpected value, causing the program to crash or malfunction. \newline \textbullet{} Considerable time can be wasted if the error is not caught by the user. & The system should validate that magnetization vectors and material IDs conform to expectations before exporting, or restrict the user input space. & F231, F233, F241, SCR4 & H7 \\
\hline
\end{tabular}
\end{table}
\end{landscape}
...
In this section, we will be analyzing the safety requirements of the system. The following list will break down potential safety requirements, their associated hazards, and their priority.

\begin{table}[H]
    \centering
    \renewcommand{\arraystretch}{1.2}
    \caption{Expanded Table of Symbolic Constants}
    \label{tab:sym}
    \begin{tabular}{|l|c|}
        \hline 
...
Note that \autoref{tab:sym} includes constants from SRS S.2, with the addition of new constants derived during the hazard analysis process.

\begin{enumerate}
\renewcommand{\labelenumi}{SCR \arabic{enumi}.}

\item \hypertarget{scr1}{}\emph{The system shall validate the imported CAD files for their format correctness before processing the model.}\\
...
\item \hypertarget{scr2}{}\emph{The system shall check if the device has enough memory and GPU capabilities before attempting to render 3D models and voxel grids.}\\
    ...
    \quad {\bf Priority:} Low

\item \hypertarget{scr3}{}\emph{The system shall maintain real-time synchronization between the UI component of selected/edited voxels and the backend data model.}\\
...
\item \hypertarget{scr4}{}\emph{The system shall validate all magnetization and material properties before exporting the file containing the metadata for all voxels.}\\
    ...
    \quad {\bf Fit Criterion:} \texttt{EXPORT\_COMPLETENESS\_THRESH} of exported files must contain complete and correct magnetization and material property data for all voxels.\\
    \quad {\bf Associated Hazards:} H4, H7\\
    \quad {\bf Priority:} Low

\item \hypertarget{scr5}{}\emph{The system shall validate the metadata file format and structure before exporting the file, ensuring that it is compatible with the next system that will read the file.}\\
...
\item \hypertarget{scr6}{}\emph{The system shall implement auto-saving functionality to preserve user progress and enable recovery from any system crashes or hardware failures.}\\
...
\item \hypertarget{scr7}{}\emph{The system shall provide undo/redo functionality to allow users to correct their mistakes without losing signifcant work.}\\
...
\item \hypertarget{scr8}{}\emph{The system shall limit the number of voxels that can be selected simultaneously to prevent any performance issues.}\\
...
\item \hypertarget{scr9}{}\emph{The system shall provide progress updates and timeout handling for long running operations such as importing/exporting the file, and voxelization.}\\
...
\begin{itemize}
    \item \hyperlink{scr1}{SCR 1}
    \item \hyperlink{scr3}{SCR 3}
    \item \hyperlink{scr5}{SCR 5}
    \item \hyperlink{scr6}{SCR 6}
    \item \hyperlink{scr7}{SCR 7}
\end{itemize}

Requirements that will be implemented in the future, as they are considered lower priority, and will only be implemented if time permits:
\begin{itemize}
    \item \hyperlink{scr2}{SCR 2}
    \item \hyperlink{scr4}{SCR 4}
    \item \hyperlink{scr8}{SCR 8}
    \item \hyperlink{scr9}{SCR 9}
\end{itemize}
