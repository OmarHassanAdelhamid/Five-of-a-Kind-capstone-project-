...
Internal Review &
\begin{itemize}
  \item Each requirement is unique and unambiguous
  \item Each requirement includes a priority level and justification
  \item Any requirement does not contradict with any other requirements
  \item Each requirement can be directly traced to a stakeholder need and vice versa
  \item Functional requirements are measurable and testable
  \item Non-functional requirements are measurable and testable and/or specify fit criteria, utilizing symbolic constants when necessary
  \item Safety and security requirements can be traced to an identified hazard and vice versa
  \item Each problem found is documented as a GitHub issue
\end{itemize} \\ \hline
Supervisor Review Preparation &
\begin{itemize}
  \item SRS highlights document prepared
  \item Overview of SRS walkthrough completed
  \item Next steps \& focus areas communicated and validated
  \item All supervisor feedback recorded and formatted into separate issues in GitHub
\end{itemize} \\ \hline
Continuous Verification &
\begin{itemize}
  \item Each GitHub issue is closed either via commit or by comment with a detailed explanation
  \item Appendix contains a traceability table mapping each GitHub issue to a change/comment and short explanation
  \item Each change to the SRS triggers a change to affected documents and their respective traceability tables
\end{itemize} \\ \hline
\end{tabular}
\end{table}
