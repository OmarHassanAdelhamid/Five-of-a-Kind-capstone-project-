...
\noindent This document outlines the Verification and Validation (V\&V) Plan for \textbf{AutoVox}, developed by Team 10 --- \textit{Five of a Kind}. It describes how the system will be tested to ensure that all functional, non-functional, and safety requirements defined in the Software Requirements Specification (SRS) are satisfied. The plan establishes the strategies, team roles, and procedures that will be used to verify correctness, performance, and usability of the software.\\

\noindent The document is organized into two main sections. The first section, \textbf{General Information}, provides background context including a summary of the system, testing objectives, project challenge level, and relevant supporting documents. The second section, \textbf{Plan}, outlines the structure of the V\&V process, identifies team responsibilities, and describes the approach for verifying the SRS through structured reviews, stakeholder feedback, and continuous validation throughout development.\\

\section{General Information}

The following subsections cover a basic summary of what AutoVox aims to accomplish, and what the high-level objectives of this verification and validation plan are. In addition, the challenge level of this project and the selected extra documentation that will be developed and tested under this document are covered, as well as any other relevant documentation that aids in this document's understanding. \\

\subsection{Summary}

The software system being tested is called \textbf{AutoVox}, developed by Team 10 --- \textit{Five of a Kind}. AutoVox is a desktop-based CAD enhancement tool that allows researchers and engineers to assign properties to individual voxels within a 3D model. It is designed to streamline the process of magnetization and material planning for multi-material 3D printing workflows within a research laboratory environment.\\

\noindent Currently, these objects are modeled within COMSOL, requiring researchers to tediously build models from sketches voxel-by-voxel, and assign properties to each voxel individually. AutoVox removes this bottleneck by enabling users to import pre-built CAD models (e.g., from AutoCAD), automatically slice them into voxels, and interactively select single or multiple voxels to assign or modify its properties. The software also provides per-layer visualization, undo/redo functionality, and validation mechanisms to ensure complete and accurate voxel data before export.\\

\noindent The final output is a \textbf{Java-readable metadata file} that contains per-voxel property information, which integrates directly with the custom 3D printer pipeline used in the lab. By automating voxel processing and enabling efficient property editing, AutoVox dramatically reduces the time and effort required to prepare print-ready models, ensuring a faster, more reliable, and user-friendly workflow for advanced multi-material 3D printing research.\\
...
\noindent Specifically, the objectives of V\&V are as follows:\\
...
\noindent Overall, the V\&V process aims to provide stakeholders with confidence that \textbf{AutoVox} achieves its intended purpose as a robust, user-friendly, and trustworthy software solution for voxel-based magnetization design.\\

\subsection{Challenge Level and Extras}

\noindent The challenge level for \textbf{AutoVox} is classified as \textbf{advanced}. The system integrates multiple complex subsystems, including 3D model import and voxelization, real-time voxel visualization and editing, magnetization and material assignment, and structured export to a custom printer pipeline. Each subsystem involves substantial technical and architectural considerations, such as handling large voxel datasets, maintaining UI synchronization, and ensuring data integrity across modules. These factors collectively establish the project as an advanced-level software system.\\

\noindent The extras included in this project are:
...
\noindent These extras complement the technical objectives of the project by ensuring that the final product is not only functionally correct but also accessible, intuitive, and well-documented for its target users.\\

\subsection{Relevant Documentation}

\noindent The documentation that supports the verification and validation (V\&V) process for \textbf{AutoVox} includes the Software Requirements Specification (SRS), Hazard Analysis (HA), Development Plan (DP), and Problem Statement and Goals. Each document serves a distinct purpose in guiding and supporting the testing strategy.\\

\noindent The \textbf{Software Requirements Specification (SRS)} defines all functional and non-functional requirements for the system. It provides the foundation for test case design, ensuring that every requirement can be verified through measurable criteria and traced throughout the V\&V process \citet{SRS}.\\

\noindent The \textbf{Hazard Analysis (HA)} identifies potential system hazards, their causes, and mitigation strategies. It establishes the safety requirements (SCRs) that must be verified to ensure the system operates safely and consistently under expected laboratory conditions \citet{HA}.\\

\noindent The \textbf{Development Plan (DP)} outlines the project’s workflow, coding standards, continuous integration setup, and proof-of-concept demonstration plan. It informs the structure and scheduling of testing activities by defining the methods and tools used during development \citet{DP}.\\

\noindent The \textbf{Problem Statement and Goals} document provides the original motivation, scope, and intended outcomes of the project. It ensures that the V\&V process remains aligned with the project’s high-level objectives and stakeholder expectations \citet{PS}.\\

\noindent Finally, the project’s \textbf{GitHub Repository} serves as the central hub for all implementation artifacts, testing scripts, and documentation updates: \url{https://github.com/OmarHassanAdelhamid/Five-of-a-Kind-capstone-project}\\

\noindent Together, these documents ensure that all verification and validation activities are traceable, comprehensive, and consistent with the system’s intended functionality and safety goals.\\

\section{Plan}
\noindent The \textbf{Plan} section outlines the structure and approach of the verification and validation (V\&V) process for \textbf{AutoVox}. It describes the composition of the V\&V team, their assigned roles, and responsibilities across different testing domains, ensuring that all system components are thoroughly verified and validated. This section also details the planned approach for verifying the Software Requirements Specification (SRS), including the review strategy, feedback mechanisms, and formal verification methods that will be employed to confirm that all requirements are correct, testable, and traceable throughout development.\\

\noindent The verification and validation (V\&V) efforts for \textbf{AutoVox} will be conducted by \textbf{Team 10 --- \textit{Five of a Kind}}. Each member plays a specific role to ensure that all functional, non-functional, and safety requirements are properly verified and validated. The distribution of responsibilities ensures comprehensive coverage of testing, documentation, and review activities across all project domains. Table~\ref{tab:roles} reflects these responsibilities, assigning each member a specific role.\\
...
\noindent These roles remain flexible in the case that there is notable overlap between the issues two (or more) roles focus on. For instance, if it were found that the implementation of a given feature drastically affects performance but satisfies functional requirements, some degree of coordination would need to take place between functional and non-functional testers as there would be overlap of responsibility and a need for close collaboration. Additionally, if one team member is less proficient at a specific task within their role, other members may of course provide assistance, given that the team member requesting help shares in their duties.\\

\noindent This structure ensures that all verification and validation efforts—ranging from requirement coverage and performance testing to usability evaluation and safety assurance—are systematically planned, executed, and reviewed in collaboration with the supervisor.\\

\subsection{SRS Verification}

\noindent Verification of the Software Requirements Specification (SRS) for \textbf{AutoVox} will follow a structured, multi-stage process to ensure that all requirements are correct, complete, testable, and traceable. This process includes internal reviews, supervisor meetings, peer review feedback, and continuous verification. Each activity is designed to improve requirement quality and maintain traceability between the SRS and subsequent project deliverables.\\

\subsubsection{Internal Review and Checklist Verification}

\noindent The first stage of SRS verification will involve a detailed internal review conducted by all team members. Each member will focus on a particular category of requirements:\\
...
\begin{itemize}
  ...
  \item An informal feedback session for the supervisor to provide clarifications or suggest modifications.
\end{itemize}

\noindent The feedback session will take the form of an informal interview, during which supervisor comments will be recorded for later formatting into individual GitHub issues. Comments will revolve around the SRS walkthrough and following discussion, as well as examination of the SRS itself if needed. Each comment will be linked to the corresponding requirement or section of the document and tracked until resolution. Follow-up actions will be reviewed in subsequent supervisor meetings.\\

\subsubsection{Peer Review and Class Feedback}

\noindent In addition to the supervisor review, the SRS will undergo peer evaluation during the class-wide review process. The primary reviewer team will assess the document’s clarity, consistency, and completeness. Their feedback will be formally logged and categorized according to:\\
...
\noindent All peer feedback will be integrated systematically into the SRS through a traceability table within an appendix, ensuring that each comment maps to a specific requirement and its revision status.\\

\subsubsection{Continuous Verification and Traceability Maintenance}

\noindent Throughout development, the team will maintain continuous verification of the SRS using GitHub issues, milestones, and project boards. Each requirement will be linked to corresponding design elements, test cases, and validation results. Regular bi-weekly internal reviews will verify that any new or modified requirements remain traceable and verifiable.\\

\noindent This iterative approach ensures that the SRS remains accurate, actionable, and aligned with the evolving implementation. Any changes introduced after supervisor or peer feedback will trigger updates to both the traceability matrix and the V\&V Plan to maintain consistency across documents.\\

\noindent \textbf{Documentation of bi-weekly reviews and resulting changes:} Each bi-weekly internal SRS review will be summarized in dated meeting notes stored under \texttt{docs/} (for example a short markdown or \LaTeX{} log linked from the repository documentation), recording agenda items, decisions, and follow-up actions. Outcomes that require edits to the SRS will be captured as GitHub issues referencing the affected requirement identifiers or section labels; corresponding updates will be merged via pull requests that cite those issues, with issue closures documenting what changed in the text. The appendix traceability table will be updated so that each resolved item maps to its SRS revision, alongside GitHub's issue and commit history.
...
Internal Review &
\begin{itemize}
  \item \textbf{Uniqueness and clarity.} Walk the numbered requirements list and mark any duplicate intent, vague pronouns, or missing units. \textbf{Complete when:} every requirement has a single clear statement; duplicates are merged or cross-referenced; reviewer initials and date are recorded in meeting notes or the review issue.
  \item \textbf{Priority and rationale.} \textbf{Complete when:} each requirement row or subsection states priority  and one sentence explaining why it matters to a stakeholder or hazard.
  \item \textbf{Consistency.} Build a search for conflicting terms (e.g.\ timing, file formats, error behaviour). \textbf{Complete when:} no two requirements contradict.
  \item \textbf{Stakeholder traceability.} For each requirement ID, point to a problem-goal statement, user story, or hazard ID. \textbf{Complete when:} the appendix or traceability table shows bidirectional links (stakeholder need $\leftrightarrow$ requirement ID) with no orphan IDs on either side.
  \item \textbf{Functional testability.} For each FR, state observable inputs, expected outputs, and environment (manual or automated). \textbf{Complete when:} a reviewer can draft a test title from the text without guessing; any FR that cannot be tested is rewritten or marked out-of-scope with approval.
  \item \textbf{Non-functional testability.} For each NFR, specify metric, measurement method, and threshold (numbers or symbolic constants from the SRS). \textbf{Complete when:} each NFR has a defined pass/fail rule and tool or procedure (e.g.\ timer, load script, checklist).
  \item \textbf{Safety and security (SCR) linkage.} \textbf{Complete when:} each SCR cites a Hazard Analysis ID; each hazard has at least one SCR; the mapping is listed in the SRS or hazard appendix.
  \item \textbf{Requirement--test coverage (forward).} \textbf{Complete when:} every FR, NFR, and SCR in the current SRS baseline is listed in the V\&V Plan (or test plan appendix) with at least one test case ID or explicitly marked \emph{deferred} with rationale and target milestone.
  \item \textbf{Test--requirement coverage (reverse).} \textbf{Complete when:} every planned system/integration test case in the V\&V Plan cites at least one SRS requirement ID; tests with no requirement are removed or re-scoped.
  \item \textbf{Issue logging.} \textbf{Complete when:} each defect or review finding is a GitHub issue with requirement ID(s) in the title or body, assignee, and open/closed state.
\end{itemize} \\ \hline
Supervisor Review Preparation &
\begin{itemize}
  \item \textbf{Highlights pack.} Produce a short PDF or section list: top risks, ambiguous phrases, and proposed test approaches. \textbf{Complete when:} document is shared in the meeting invite or repository path is recorded in meeting notes.
  \item \textbf{Walkthrough readiness.} Team has read the pack and assigned sections to speakers. \textbf{Complete when:} run-through completed or time-boxed agenda with section coverage checklist is checked off.
  \item \textbf{Alignment on follow-up.} \textbf{Complete when:} meeting notes list agreed decisions, owners, and due dates for SRS edits.
  \item \textbf{Feedback capture.} \textbf{Complete when:} each supervisor comment is one GitHub issue (or sub-issue) quoting the SRS sentence, with label \texttt{srs-review} and link to the meeting date.
\end{itemize} \\ \hline
Continuous Verification &
\begin{itemize}
  \item \textbf{Issue resolution.} \textbf{Complete when:} closed issues either reference a merged PR/commit that edits the SRS or a comment stating ``won't fix'' with supervisor/stakeholder agreement.
  \item \textbf{Traceability appendix.} \textbf{Complete when:} the appendix maps each closed SRS-related issue to requirement ID(s), summary of change, and revision of the SRS section.
  \item \textbf{Downstream consistency.} \textbf{Complete when:} any SRS change triggers an explicit check of Design Doc, V\&V Plan test IDs, and User Manual snippets; updates are listed in the PR description or a single changelog issue.
  \item \textbf{Regression of coverage.} After edits, re-run the forward/reverse requirement--test checks above. \textbf{Complete when:} no new orphan requirements or orphan tests relative to the updated baseline.
\end{itemize} \\ \hline
\end{tabular}
\end{table}

\noindent\textbf{Summary:} This structured, multi-level verification process guarantees that the SRS for \textbf{AutoVox} is validated both internally and externally. Through continuous review, stakeholder feedback, and traceability enforcement, the team ensures that all requirements are precise, measurable, and fully verifiable before system testing begins.\\

\subsection{Design Verification}

\noindent The design verification will primarily focus on the system framework and integrated architecture that details the system implementation. Verification will take place through structured classmate and supervisor reviews. Reviews conducted by classmates will be designed to emphasize the interface design that permits usage of external libraries, internal refactoring and the primary database structure that encapsulates all voxel metadata. Feedback will be created into GitHub issues and discussed amongst the group in order to determine the best course of action to effectively integrate the feedback in subsequent development cycles.\\
...
\noindent Table~\ref{tab:des} details the checklist that will be utilized during design verification to ensure all necessary tasks are completed.\\
...
Core Architecture &
\begin{itemize}
\item \textbf{Modularity and boundaries.} List major subsystems (e.g.\ Import, Visualization, Editing, Export, persistence) and their public interfaces. \textbf{Complete when:} a diagram or table shows modules, dependencies, and data flow; no hidden cycles without justification.
\item \textbf{Conventions.} \textbf{Complete when:} the design doc names language/runtime (Python/TypeScript), packaging layout, and points to enforced style configs (e.g.\ PEP~8, ESLint) used in CI.
\item \textbf{Patterns and rationale.} State chosen patterns (e.g.\ layered architecture, repository for voxel data) and alternatives rejected. \textbf{Complete when:} each pattern ties to at least one SRS concern (performance, testability, safety).
\item \textbf{Failure and error paths.} Describe how errors propagate (user-visible vs.\ logged). \textbf{Complete when:} failure modes for import, slice, save, and export map to SRS error/robustness expectations.
\end{itemize} \\ \hline
IDE Integration &
\begin{itemize}
\item \textbf{Voxel slicing library.} Document API surface used, version pin, and threading model. \textbf{Complete when:} a smoke test or contract test is identified in the V\&V Plan that fails if the library contract breaks.
\item \textbf{3D/UI stack.} State renderer (e.g.\ Three.js) responsibilities vs.\ application state. \textbf{Complete when:} critical UI flows in the design match F221--F223 and have corresponding manual or automated checks in the plan.
\item \textbf{Build and dev experience.} \textbf{Complete when:} README or design doc lists how to run backend, frontend, and tests locally; matches CI job names.
\end{itemize} \\ \hline
Database Integrity &
\begin{itemize}
\item \textbf{Schema and entities.} Show tables/collections, keys, and voxel metadata fields. \textbf{Complete when:} schema matches persistence needs for F231--F239 and NF232 where applicable; migration approach stated.
\item \textbf{Operations.} For create/read/update/delete paths, note transactions and consistency expectations. \textbf{Complete when:} at least one test or script in the V\&V Plan validates round-trip save/load for a representative project.
\item \textbf{Backup and recovery.} \textbf{Complete when:} autosave frequency and crash-recovery behaviour align with NFR/SCR text or gaps are filed as issues.
\end{itemize} \\ \hline
External Data Integrity &
\begin{itemize}
\item \textbf{File formats.} List supported CAD/export formats, field definitions, and version handling. \textbf{Complete when:} each format maps to Import/Export SRS requirements and has a positive and negative test case (invalid file) where relevant.
\item \textbf{Interchange validation.} \textbf{Complete when:} golden sample files are named in repo or issue and used in regression tests.
\end{itemize} \\ \hline
Traceability (SRS and tests) &
\begin{itemize}
\item \textbf{Design $\rightarrow$ SRS.} For each major component or interface in the design, cite requirement ID(s) it implements. \textbf{Complete when:} every substantive design subsection lists SRS IDs; every SRS requirement allocated to software has an assigned component or a ``TBD'' issue with owner.
\item \textbf{SRS $\rightarrow$ design $\rightarrow$ test.} \textbf{Complete when:} for each FR/NFR/SCR, there is a chain: requirement ID $\rightarrow$ design element $\rightarrow$ test case ID in the V\&V Plan (or documented deferral).
\item \textbf{No orphan tests.} \textbf{Complete when:} each design-level or integration test in the plan references at least one SRS requirement; stray exploratory tests are either linked or removed from scope.
\item \textbf{No untested requirements.} \textbf{Complete when:} a coverage matrix (or module trace tables in the V\&V Plan) shows every current SRS requirement ID with $\geq$1 test or an explicit waiver with sign-off.
\item \textbf{Change control.} \textbf{Complete when:} design revisions update the same trace matrix when requirements or tests change; PRs that touch design reference the trace issue or appendix row.
\end{itemize} \\ \hline
\end{tabular}
\end{table}
...
\noindent Table~\ref{tab:vnv} details a checklist that will be utilized during verification of the V\&V plan for reference.\\
...
Coverage &
\begin{itemize}
\item Quantitative metrics stated explicitly (e.g.\ line/branch coverage targets per milestone, or interaction-count baselines for UI flows).
\item Testing domains listed (backend, frontend, integration, usability, performance) with primary tools named (Pytest, Jest, manual scripts).
\item Coverage cases named: which SRS subsystems (Import, Visualization, Editing, Export) have at least one system or unit test trace.
\item Each major milestone includes a short coverage summary table or CI artifact reference, not only narrative text.
\end{itemize} \\ \hline
Testing Methodology &
\begin{itemize}
\item Baseline success metrics defined per test type.
\item Tests classified (automated unit/integration, manual exploratory, performance, usability).
\item Mutation or fault-injection scope identified where used.
\end{itemize} \\ \hline
Traceability &
\begin{itemize}
\item GitHub issues or project labels link tests to requirement IDs; closing an issue references the test run or commit.
\item Peer or TA feedback on the V\&V plan logged as issues with accept/reject and document revision tracked.
\end{itemize} \\ \hline
\end{tabular}
\end{table}
...
\item interface responsiveness for multi-perspective navigation and partition switching (F223), measured against the interaction-count or latency budgets documented in the performance tests (not an abstract ``optimality'' claim)
\item large database interaction (NF232, SCR3)
\end{itemize}

\noindent Table~\ref{tab:impl} details a checklist that will be utilized throughout implementation verification to ensure completeness.\\
...
Functionality &
\begin{itemize}
\item Test case sets mapped to SRS requirement IDs (S.2); each automated test name or description references the FR/NFR it exercises where practical.
\item Backend: Pytest suites pass for voxel editing, persistence, and export behaviours tied to F231--F239.
\item Frontend: Jest (and manual smoke steps where automation is impractical) pass for viewer, partition navigation, and layer tools tied to F221--F223.
\item Regression: known small STL/voxel test models re-run after viewer or slicing changes to confirm end-to-end consistency.
\end{itemize} \\ \hline
Quality &
\begin{itemize}
\item At least one structured code review per PR touching application logic; reviewers apply the numbered workflow in Section~\ref{sec:code-review-steps}.
\item Static analysis: Python PEP~8 enforcement, TypeScript/ESLint clean for frontend.
\item Security-focused inspection of file import/export and path handling before release candidates (SCR1, SCR4, SCR5).
\end{itemize} \\ \hline
Performance &
\begin{itemize}
\item Import/export throughput or timing benchmarks recorded against NF212/NF242 expectations for representative file sizes.
\item UI: interaction counts or latency for core flows (import, rotate/zoom, partition change, export) compared to documented baselines from the VnV performance tests.
\item Database: query or transaction benchmarks for large projects against NF232/SCR3 where applicable.
\end{itemize} \\ \hline
Traceability &
\begin{itemize}
\item GitHub issues and PRs reference requirement IDs; closed issues state what was verified or deferred.
\item Coverage reports (pytest-cov, Jest) archived or linked in CI artifacts for major milestones.
\end{itemize} \\ \hline
\end{tabular}
\end{table}
...
\noindent\textbf{Unit Testing Framework (backend):} Since the backend of our system will be coded in Python, the primary testing framework used will be PyTest. This framework was chosen due to its flexible testing framework that can be scaled to support complex function testing. \\
...
\begin{itemize}
\item create a new project with a given CAD file (F221)
...
\noindent This section outlines the tests for verifying both functional and nonfunctional requirements of the system. It helps ensure that the system meets stakeholders' expectations. This section includes tests covering the essential aspects of the systems requirements.

\subsection{Tests for Functional Requirements}

\noindent The subsections below outline tests corresponding to functional requirements in the SRS.
...
\item{test-FR-EM-15 Manual Save Functionality\\}
\label{sec:t-em-15}
...
\textbf{Input:} User selects option to Save project.

\textbf{Output:} Changes are saved successfully and confirmation is displayed.

\textbf{Test Case Derivation:} Ensures that the user can manually preserve their progress should the situation arise, as per F2311.

\textbf{How test will be performed:} Make changes to the property assignments in the model, use the save option to save the project, and verify the changes are reflected in the project file.
...
\noindent This section covers the tests for ensuring the Export Manager correctly
...
\noindent The following subsections outline tests corresponding to non-functional requirements in the SRS. Instead of organizing these tests by each component, tests are grouped into common non-functional areas of testing.\\
...
\textbf{Covers:} NF213, NF231, NF243
					
\textbf{Initial State:} Application open
					
\textbf{Input:} User completes core tasks (import file, material/magnetization assignment, export file)
					
\textbf{Output:} USABILITY\_PERCENTAGE\% of users tested complete tasks in less than MAX\_INTERACTIONS.
...
or not (e.g. pass/fail). Investigation in how to reword prompts will take place for messages that under USABILITY\_PERCENTAGE\% of users
do not understand.
...
\noindent The following tables show the traceability between test cases and their corresponding functional requirements, and demonstrate that complete coverage is achieved.\\
...
\begin{table}[H]
\centering
\caption{Trace Between Import Manager Tests and Corresponding Requirements}
\label{tab:imt}
\small
...
\hyperref[sec:t-im-1]{test-FR-IM-1} & 
\xbox & \obox & \obox & \obox & \obox & \obox \\

\hyperref[sec:t-im-2]{test-FR-IM-2} & 
\obox & \xbox & \obox & \obox & \obox & \obox \\

\hyperref[sec:t-im-3]{test-FR-IM-3} & 
\xbox & \obox & \obox & \obox & \obox & \obox \\

\hyperref[sec:t-im-4]{test-FR-IM-4} & 
\obox & \obox & \xbox & \obox & \obox & \obox \\

\hyperref[sec:t-im-5]{test-FR-IM-5} & 
\obox & \obox & \obox & \xbox & \obox & \obox \\

\hyperref[sec:t-im-6]{test-FR-IM-6} & 
\obox & \obox & \obox & \obox & \xbox & \obox \\

\hyperref[sec:t-im-7]{test-FR-IM-7} & 
\obox & \obox & \obox & \obox & \obox & \xbox \\
...
\begin{table}[H]
\centering
\caption{Trace Between Visualization Manager Tests and Corresponding Requirements}
\label{tab:vmt}
\small
\setlength{\tabcolsep}{3pt}
\begin{tabular}{l|*{8}{c}}
\toprule
\textbf{Reqs:} 
& F211 & F221 & F222 & F223 & F224 & F225 & F226 & F227 \\
\midrule

\hyperref[sec:t-vm-1]{test-FR-VM-1} & 
\obox & \xbox & \obox & \obox & \obox & \obox & \obox & \obox\\

\hyperref[sec:t-vm-2]{test-FR-VM-2} & 
\obox & \obox & \xbox & \obox & \obox & \obox & \obox & \obox\\

\hyperref[sec:t-vm-3]{test-FR-VM-3} & 
\obox & \obox & \obox & \xbox & \obox & \obox & \obox & \obox\\

\hyperref[sec:t-vm-4]{test-FR-VM-4} & 
\obox & \obox & \obox & \obox & \xbox & \obox & \obox & \obox\\

\hyperref[sec:t-vm-5]{test-FR-VM-5} & 
\obox & \obox & \obox & \obox & \obox & \xbox & \obox & \obox\\

\hyperref[sec:t-vm-6]{test-FR-VM-6} & 
\obox & \obox & \obox & \obox & \obox & \obox & \xbox & \obox\\

\hyperref[sec:t-vm-7]{test-FR-VM-7} & 
\obox & \obox & \obox & \obox & \obox & \obox & \obox & \xbox\\

\hyperref[sec:t-int-1]{test-FR-INT-1} & 
\xbox & \xbox & \obox & \obox & \obox & \obox & \obox & \obox\\
...
\begin{table}[H]
\centering
\caption{Trace Between Editing Manager Tests and Corresponding Requirements}
\label{tab:emt}
\small
\setlength{\tabcolsep}{3pt}
...
\toprule
\textbf{Reqs:} 
& F231 & F232 & F233 & F234 & F235 & F236 & F237 & F238 & F239 & F2310 & F2311\\
\midrule

\hyperref[sec:t-em-1]{test-FR-EM-1} & 
\xbox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox\\

\hyperref[sec:t-em-2]{test-FR-EM-2} & 
\xbox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox\\

\hyperref[sec:t-em-3]{test-FR-EM-3} & 
\obox & \xbox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox\\

\hyperref[sec:t-em-4]{test-FR-EM-4} & 
\obox & \obox & \xbox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox\\

\hyperref[sec:t-em-5]{test-FR-EM-5} & 
\obox & \obox & \xbox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox\\

\hyperref[sec:t-em-6]{test-FR-EM-6} & 
\obox & \obox & \obox & \xbox & \obox & \obox & \obox & \obox & \obox & \obox & \obox\\

\hyperref[sec:t-em-7]{test-FR-EM-7} & 
\obox & \obox & \obox & \obox & \xbox & \obox & \obox & \obox & \obox & \obox & \obox\\

\hyperref[sec:t-em-8]{test-FR-EM-8} & 
\obox & \obox & \obox & \obox & \obox & \xbox & \obox & \obox & \obox & \obox & \obox\\

\hyperref[sec:t-em-9]{test-FR-EM-9} & 
\obox & \obox & \obox & \obox & \obox & \obox & \xbox & \obox & \obox & \obox & \obox\\

\hyperref[sec:t-em-10]{test-FR-EM-10} & 
\obox & \obox & \obox & \obox & \obox & \obox & \obox & \xbox & \obox & \obox & \obox\\

\hyperref[sec:t-em-11]{test-FR-EM-11} & 
\obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \xbox & \obox & \obox\\

\hyperref[sec:t-em-12]{test-FR-EM-12} & 
\obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \xbox & \obox & \obox\\

\hyperref[sec:t-em-13]{test-FR-EM-13} & 
\obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \xbox & \obox\\

\hyperref[sec:t-em-14]{test-FR-EM-14} & 
\obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \xbox & \obox\\

\hyperref[sec:t-em-15]{test-FR-EM-15} & 
\obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \xbox\\
...
\begin{table}[H]
\centering
\caption{Trace Between Export Manager Tests and Corresponding Requirements}
\label{tab:xmt}
\small
\setlength{\tabcolsep}{3pt}
\begin{tabular}{l|*{5}{c}}
...
\hyperref[sec:t-xm-1]{test-FR-XM-1} & 
\xbox & \obox & \obox & \obox & \obox \\

\hyperref[sec:t-xm-2]{test-FR-XM-2} & 
\xbox & \obox & \obox & \obox & \obox \\

\hyperref[sec:t-xm-3]{test-FR-XM-3} & 
\obox & \xbox & \obox & \obox & \obox \\

\hyperref[sec:t-xm-4]{test-FR-XM-4} & 
\obox & \obox & \xbox & \obox & \obox \\

\hyperref[sec:t-xm-5]{test-FR-XM-5} & 
\obox & \obox & \obox & \xbox & \obox \\

\hyperref[sec:t-xm-6]{test-FR-XM-6} & 
\obox & \obox & \obox & \obox & \xbox \\

\hyperref[sec:t-xm-7]{test-FR-XM-7} & 
\xbox & \obox & \obox & \obox & \obox \\

\hyperref[sec:t-xm-8]{test-FR-XM-8} & 
\xbox & \obox & \obox & \obox & \obox \\
...
\begin{table}[H]
\centering
\caption{Trace Between Non-Functional Tests and Corresponding Requirements}
\label{tab:nft}
\small
\setlength{\tabcolsep}{3pt}
...
\toprule
\textbf{Reqs:} 
& NF211 & NF212 & NF213 & NF221 & NF222 & NF223 & NF231 & NF232 & NF241 & NF242 & NF243 & GEN1 & GEN2 & E3.1 & E3.2 & E3.3 & E3.4 & S3 & S4 & DP.10 \\
\midrule

\hyperref[sec:t-nf-1]{test-US-1} & 
\obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \xbox & \obox & \obox\\

\hyperref[sec:t-nf-2]{test-US-2} & 
\obox & \obox & \obox & \obox & \obox & \xbox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox\\

\hyperref[sec:t-nf-3]{test-US-3} & 
\obox & \obox & \xbox & \obox & \obox & \obox & \xbox & \obox & \obox & \xbox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox\\

\hyperref[sec:t-nf-4]{test-US-4} & 
\obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \xbox & \obox & \obox\\

\hyperref[sec:t-nf-5]{test-US-5} & 
\obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \xbox & \obox\\

\hyperref[sec:t-nf-6]{test-US-6} & 
\obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \xbox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox\\

\hyperref[sec:t-nf-7]{test-PS-1} & 
\xbox & \xbox & \obox & \obox & \xbox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox\\

\hyperref[sec:t-nf-8]{test-PS-2} & 
\obox & \xbox & \obox & \xbox & \obox & \obox & \obox & \xbox & \obox & \xbox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox\\

\hyperref[sec:t-nf-9]{test-PS-3} & 
\obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \xbox & \obox & \obox & \obox\\

\hyperref[sec:t-nf-10]{test-CO-1} & 
\obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \xbox & \obox & \obox & \obox & \obox & \obox & \obox\\

\hyperref[sec:t-nf-11]{test-CO-2} & 
\obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \xbox & \xbox & \obox & \obox & \obox & \obox\\

\hyperref[sec:t-nf-12]{test-CO-3} & 
\obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \xbox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox\\

\hyperref[sec:t-nf-13]{test-CO-4} & 
\obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \xbox\\

\hyperref[sec:t-nf-14]{test-MA-1} & 
\obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox & \xbox & \obox & \obox & \obox & \obox & \obox & \obox & \xbox\\
...
\noindent To avoid repeating repetitive information, to view the full set of unit tests, view the backend test cases within 
\href{https://github.com/OmarHassanAdelhamid/Five-of-a-Kind-capstone-project-/tree/main/src/backend/tests}{src/backend/tests}
and frontend tests within \href{https://github.com/OmarHassanAdelhamid/Five-of-a-Kind-capstone-project-/tree/main/src/frontend}{src/frontend} 
(all test files have test as suffix). For backend unit tests, cases were selected with the aim
to execute each if statement or possibility either way, as well as triggering foreseeable error cases that should be verifiably caught. 
Edge cases were also identified by inspecting each function, investigating potential misinputs, and creating a test case for the specific
misinput to observe the behavior. Testing with a combination of mocks and real files was also performed to balance testing upper-level modules
in isolation, as well as ensuring lower-level modules can operate on the actual files AutoVox deals with. \\

\noindent Some major test classes are as follows:\\

\begin{itemize}
  \item \textbf{Model Editing Tests}: Testing that both valid and invalid model edits to properties and additions/deletions are handled correctly.
  \item \textbf{Model Surface Finding Tests}: A set of tests to ensure the correct and minimal number of voxels are being returned from models for rendering.
  \item \textbf{Model Tracking Tests:} Testing that valid and invalid requests from the model are returned as expected.
  \item \textbf{Edit Router Tests:} Testing of a crucial interaction controller router implementation through mocking. Aimed to cover edge cases that would likely not happen from the frontend to verify that odd behavior would not occur.
  \item \textbf{Project Management Router Tests:} A set of tests for another important interaction controller implementation through mocking that covers that the correct services and functions are being called during project creation.
\end{itemize}

\noindent Test cases were organized in frontend and backend into one file per module, with folders for each set of modules (e.g. in the backend, 
test files were split between routers and services). Names of all test cases were also made to be self-descriptive, not requiring additional documentation within the code to explain what they
test (e.g. the entire test case is in the name of the function). 
...
This section illustrates all symbolic parameters used in the above system tests in one place for maintenance and consultation ease.
...
\begin{itemize}
  ...
  \item \textbf{USABILITY\_PERCENTAGE} = 90\%
