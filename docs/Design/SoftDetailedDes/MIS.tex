...
April 6, 2026 & 2.1 & Revision 1 by All\\
\bottomrule
\end{tabularx}
...
\noindent 
\caption{Datatypes Summary}\label{TblDT}
...
Finally, \progname \ uses some enums and refers to some derived types via other aliases for 
ease of notation and understanding. For instance, Vector3D and Voxel are both aliases for a tuple
with 3 real values; UIEvent, VisualUpdate, Action, propertyName, and DataType are each enums.  
...
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
\midrule
\texttt{InteractionController} & \texttt{initialView: View} & \texttt{self} & None \\ 
...
\subsubsection{Access Routine Semantics}
\texttt{InteractionController(initialView: View) -> self}
\begin{itemize}
  \item \textbf{Transition:} Creates a new InteractionController object and initializes the first view the user begins on.
  \item \textbf{Output:} \texttt{self} - a reference to itself
  \item \textbf{Exceptions:} None
\end{itemize}
...
\begin{center}
\renewcommand{\arraystretch}{1.55}
\begin{tabular}{p{5.5cm} p{3cm} p{3cm} p{2.5cm}}
\hline
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\ \hline
\texttt{VisualizationStateManager} & None & \texttt{self} & None \\
\texttt{getCurrentView} & None & \texttt{str} & None \\
\texttt{setView} & \texttt{state: str} & \texttt{void} & None \\
\hline
\end{tabular}
\end{center}
...
\subsubsection{Access Routine Semantics}
VisualizationStateManager() $\rightarrow$ self
\begin{itemize}
  \item Transition: Creates a new VisualizationStateManager object and initializes all state variables to default values
  \item Output: self - a reference to itself
...
\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{p{6cm} p{4.5cm} p{2cm} p{3.5cm}}
\toprule
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
\midrule
\texttt{ModelManager} & voxelGrid: ModelStructure & self & None \\
\texttt{updateVoxelMaterials} & voxels: list[Vector3D], \newline material: int & None & ValueError \\
\texttt{updateVoxelMagnetizations} & voxels: list[Vector3D], \newline magnetization: Vector3D & None & ValueError \\
\texttt{addVoxels} & voxels: list[Vector3D] & None & ValueError \\
\texttt{deleteVoxels} & voxels: list[Vector3D] & None & ValueError \\
% \texttt{addVoxel} & coord: Vector3D, material: int, magnetization: Vector3D & void & InvalidInputError \\
% \texttt{removeVoxel} & coord: Vector3D & void & NotFoundError \\
% \texttt{modifyVoxel} & coord: Vector3D, material: int, magnetization: Vector3D & void & NotFoundError \\
...
\subsubsection{State Variables}
\begin{itemize}
  \item \texttt{voxelGrid}: ModelStructure — stores all voxel elements and layer mapping.
...
ModelManager(voxelGrid: ModelStructure) $\rightarrow$ self
\begin{itemize}
  \item Transition: Creates a new ModelManager object and loads an initial ModelStructure to be operated on
  \item Output: self - a reference to itself
  \item Exceptions: None.
\end{itemize}

updateVoxelMaterials(voxels: list[Vector3D], material: int) $\rightarrow$ None
\begin{itemize}
  \item Transition: Updates all voxels passed to the passed material.
  \item Output: None
  \item Exceptions: ValueError if any voxel does not exist.
\end{itemize}

updateVoxelMagnetizations(voxels: list[Vector3D], magnetization: Vector3D) $\rightarrow$ None
\begin{itemize}
  \item Transition: Updates all voxels passed to the passed magnetization.
  \item Output: None
  \item Exceptions: ValueError if any voxel does not exist.
\end{itemize}

addVoxels(voxels: list[Vector3D]) $\rightarrow$ None
\begin{itemize}
  \item Transition: Adds set of voxels to the model with default properties.
  \item Output: None
  \item Exceptions: ValueError if any voxel already exists within the model.
\end{itemize}

deleteVoxels(voxels: list[Vector3D]) $\rightarrow$ None
\begin{itemize}
  \item Transition: Removes set of voxels from the model.
  \item Output: None
  \item Exceptions: ValueError if any voxels does not exist.
\end{itemize}

% addVoxel(coord: Vector3D, material: int, magnetization: Vector3D) $\rightarrow$ void
% \begin{itemize}
...
% removeVoxel(coord: Vector3D) $\rightarrow$ void
% \begin{itemize}
...
% modifyVoxel(coord: Vector3D, material: int, magnetization: Vector3D) $\rightarrow$ void
% \begin{itemize}
...
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
\midrule
\texttt{HistoryManager} & None & \texttt{self} & None \\
...
\subsubsection{Access Routine Semantics}
\texttt{HistoryManager() -> self}
\begin{itemize}
  \item \textbf{Transition:} Creates new HistoryManager object and initializes history and redo stacks.
  \item \textbf{Output:} \texttt{self} - a reference to itself
  \item \textbf{Exceptions:} None
\end{itemize}
...
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
\midrule
\texttt{findSurface} & None & list[Voxel] & None \\
\texttt{getLayerDirectory} & None & list[int] & None \\
\texttt{getLayer} & layerIndex: int & list[Voxel] & ValueError \\
\texttt{getMagnetizedVoxels} & layerIndex: int & list[Voxel] & ValueError \\
\texttt{getFullVoxels} & voxels: list[Vector3D] & list[Voxel] & ValueError \\

% \texttt{queryByMaterial} & materialType: int & list[Voxel] & InvalidMaterialError \\
% \texttt{queryBySelection} & isSelected: bool & list[Voxel] & None \\
% \texttt{queryByRegion} & bounds: tuple(Vector3D, Vector3D) & list[Any] & InvalidBoundsError \\
...
\texttt{findSurface() -> list[Voxel]}
\begin{itemize}
  \item \textbf{Transition:} Returns minimal set of voxels representing the outermost surface of the model.
  \item \textbf{Output:} \texttt{voxels: list[Voxel]} - indicated set of voxels
  \item \textbf{Exceptions:} None
\end{itemize}
\texttt{getLayerDirectory() -> list[int]}
\begin{itemize}
  \item \textbf{Transition:} Returns list of all layers in the model.
  \item \textbf{Output:} \texttt{layers: list[int]}
  \item \textbf{Exceptions:} None
\end{itemize}
\texttt{getLayer(layerIndex: int) -> list[Voxel]}
\begin{itemize}
  \item \textbf{Transition:} Returns list of all voxels present within a layer.
  \item \textbf{Output:} \texttt{voxels: list[Voxel]}
  \item \textbf{Exceptions:} \texttt{ValueError} - if layer index passed is not present within the model.
\end{itemize}
\texttt{getMagnetizedVoxels(layerIndex: int) -> list[Voxel]}
\begin{itemize}
  \item \textbf{Transition:} Returns list of voxels with magnetization within a layer.
  \item \textbf{Output:} \texttt{voxels: list[Voxel]}
  \item \textbf{Exceptions:} \texttt{ValueError} - if layer index passed is not present within the model.
\end{itemize}
\texttt{getFullVoxels(voxels: list[Vector3D]) -> list[Voxel]}
\begin{itemize}
  \item \textbf{Transition:} Returns list of \texttt{Voxels} given a set of voxel coordinates.
  \item \textbf{Output:} \texttt{voxels: list[Voxel]}
  \item \textbf{Exceptions:} \texttt{ValueError} - if any voxel coordinate passed is not present within the model.
\end{itemize}
...
\texttt{HighlightManager} &
highlightColourMap: dict[str] = str &
self &
None \\
...
\subsubsection{Assumptions}
\begin{itemize}
  \item Semantic keys must correspond to valid entries stored within ModelStructure and elsewhere.
  \item Colour values are valid colour representations.
\end{itemize}

\subsubsection{Access Routine Semantics}

HighlightManager(highlightColourMap: dict[str] = str) $\rightarrow$ self
\begin{itemize}
  \item Transition: Creates new HighlightManager object and initializes highlight mapping dictionary
  \item Output: self -  a reference to itself.
  \item Exceptions: None.
\end{itemize}
...
\begin{itemize}
\item \texttt{MAX\_VOXELS}: int = 13996800000 - Maximum number of voxels allowed
\item \texttt{MAX\_LAYERS}: int = 518400 - Maximum number of layers allowed
\end{itemize}
...
\begin{center}
\renewcommand{\arraystretch}{1.55}
\begin{tabular}{p{6cm} p{4cm} p{3.5cm} p{2cm}}
\hline
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
\hline
\texttt{validateExportReadiness} & model: ModelStructure & bool & None \\
\texttt{checkPrinterCompatibility} & model: ModelStructure & bool & None \\
\texttt{checkPropertyCompleteness} & model: ModelStructure & list[str] & None \\
...
validateExportReadiness(model: ModelStructure) $\rightarrow$ bool
\begin{itemize}
  ...
  \item Output: bool representing the result of export readiness validation.
  \item Exceptions: None.
\end{itemize}

checkPrinterCompatibility(model: ModelStructure) $\rightarrow$ bool
\begin{itemize}
...
checkPropertyCompleteness(model: ModelStructure) $\rightarrow$ list[str]
\begin{itemize}
...
\begin{itemize}
\item \texttt{countVoxels(model: ModelStructure) -> int}: Counts the total number of voxels in the model
\item \texttt{countLayers(model: ModelStructure) -> int}: Counts the total number of layers in the model
...
exportProject(model: ModelStructure, exportPath: str, format: str) $\rightarrow$ void
\begin{itemize}
...
transformToExportFormat(model: ModelStructure, format: str) $\rightarrow$ Any
\begin{itemize}
...
\texttt{addVoxel} & ix: int, iy: int, iz: int & None & ValueError \\
\texttt{deleteVoxel} & ix: int, iy: int, iz: int & None & ValueError \\
\texttt{getProperties} & ix: int, iy: int, iz: int & material: int, \newline magnetization: Vector3D & ValueError \\
\texttt{setProperties} & ix: int, iy: int, iz: int, \newline material: int, \newline magnetization: Vector3D & None & ValueError \\
\texttt{setMaterial} & ix: int, iy: int, iz: int, \newline material: int & None & ValueError \\
\texttt{setMagnetization} & ix: int, iy: int, iz: int, \newline magnetization: Vector3D & None & ValueError \\
...
\subsubsection{State Variables}
\begin{itemize}
  \item \texttt{voxelGrid: list[Voxel]} - Three-dimensional grid storing voxel data
...
\begin{itemize}
  \item \textbf{Transition:} Initializes \texttt{voxelGrid} to empty and initializes partition list
  \item \textbf{Output:} \texttt{self} - reference to created \texttt{ModelStructure} object
  \item \textbf{Exceptions:} None
\end{itemize}
\texttt{addVoxel(ix: int, iy: int, iz: int) -> None}
\begin{itemize}
  \item \textbf{Transition:} Adds voxel with specified indexes.
  \item \textbf{Output:} None
  \item \textbf{Exceptions:} \texttt{ValueError} if a voxel already exists at this index.
\end{itemize}
\texttt{deleteVoxel(ix: int, iy: int, iz: int) -> None}
\begin{itemize}
  \item \textbf{Transition:} Removes voxel at specified indexes
  \item \textbf{Output:} None
  \item \textbf{Exceptions:} \texttt{ValueError} if the voxel specified does not exist.
\end{itemize}
\texttt{getProperties(ix: int, iy: int, iz: int) -> material: int, magnetization: Vector3D}
\begin{itemize}
  \item \textbf{Transition:} Returns the material and magnetization of the voxel at specified indexes.
  \item \textbf{Output:} \texttt{material: int, magnetization: Vector3D}
  \item \textbf{Exceptions:} \texttt{ValueError} if the voxel specified does not exist.
\end{itemize}
\texttt{setProperties(ix: int, iy: int, iz: int, material: int, magnetization: Vector3D) -> None}
\begin{itemize}
  \item \textbf{Transition:} Sets the material and magnetization of the voxel at specified indexes.
  \item \textbf{Output:} None
  \item \textbf{Exceptions:} \texttt{ValueError} if the voxel specified does not exist.
\end{itemize}
\texttt{setMaterial(ix: int, iy: int, iz: int, material: int) -> None}
\begin{itemize}
  \item \textbf{Transition:} Sets the material of the voxel at specified indexes.
  \item \textbf{Output:} None 
  \item \textbf{Exceptions:} \texttt{ValueError} if the voxel specified does not exist.
\end{itemize}
\texttt{setMagnetization(ix: int, iy: int, iz: int, magnetization: Vector3D) -> None}
\begin{itemize}
  \item \textbf{Transition:} Sets the magnetization of the voxel at specified indexes.
  \item \textbf{Output:} None
  \item \textbf{Exceptions:} \texttt{ValueError} if the voxel specified does not exist.
\end{itemize}
...
\subsubsection{Local Functions}
\texttt{centreStructure() -> None}
\begin{itemize}
  \item \textbf{Transition:} Centres the structure about (0, 0, 0).
  \item \textbf{Output:} None
  \item \textbf{Exceptions:} None
\end{itemize}
...
\begin{itemize}
  ...
  \item \texttt{FIELD\_ORDER}: list[str] — ordered list of field names in export format: [``x'', ``y'', ``z'', ``materialId'', ``magnetizationPolar'', ``magnetizationAzimuth''].
...
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
\midrule
\texttt{ExportStructure} & None & self & None \\ 
...
ExportStructure() $\rightarrow$ self
\begin{itemize}
  \item Transition: Creates a new ExportStructure object
  \item Output: self - a reference to itself
  \item Exceptions: None.
\end{itemize}
...
\subsubsection*{1. After implementing another team's module, what did the team learn?}
Through the implementation of the other team’s module, the team gained a deeper understanding of how a modular architecture translates from specification into concrete code. In particular, the MIS/MG interface---comprising the \texttt{logEvent} access program, exported constants, and the \texttt{LogWriteException}---demonstrated the practical enforcement of strict information hiding principles. This required the team to adhere closely to the provided specification rather than relying on familiar third-party logging libraries or personal design preferences. As a result, the team learned the importance of specification-driven development in modular systems.

\subsubsection*{2. Were all the details needed to implement the module included in the documentation, or did the team need to make assumptions or ask questions?}
The documentation fully specified the public interface of the module. It clearly defined the exported constants (\texttt{LOG\_FILE\_PATH}, \texttt{LOG\_TYPE = [``Event'', ``Error'']}), the access program \texttt{logEvent(eventMssg: str, logType: str)}, the semantics of the \texttt{LogWriteException}, and the module’s role within the \texttt{BackendController}. This information was sufficient to complete a functional implementation without contacting the original team.

However, several internal design details were not explicitly documented and required reasonable assumptions. These included the default log file path (implemented using an environment variable fallback), the handling of unknown log types, the log message formatting, and the choice to use standard library file I/O in append mode with immediate flushing.

Overall, the team was able to complete the implementation using only the documentation and the surrounding backend structure. In a real-world project, the team would seek clarification on aspects such as log rotation strategy, production log file locations, and the strictness of \texttt{LOG\_TYPE} validation to ensure long-term maintainability and scalability.


\subsubsection*{3. Which of your design decisions stemmed from speaking to your client(s)
or a proxy (e.g. your peers, stakeholders, potential users)? For those that
were not, why, and where did they come from?} Several key design decisions were influenced by discussions with teammates acting as proxies for end users, as well as guidance provided by the TA during feedback and review sessions. The TA’s input helped reinforce the importance of clear module boundaries, low coupling, and well defined responsibilities, which directly shaped how the system was decomposed into modules. Feedback from peers also emphasized the need for maintainability and clarity over unnecessary architectural complexity. Design decisions that did not stem directly from client or TA input were informed by course material, software architecture principles discussed in lectures, and the team’s prior experience with similar systems.
...
\subsubsection*{What was this experience like? Are there things in your documentation you could have changed to make the process go more smoothly for when an ‘outsider’ completes some of the implementation?} 

To increase the likelihood that our peers would be able to produce usable and insightful code, we chose one of the more straightforward modules. In the end, I think that was the best choice, as their team was able to understand and closely follow our MIS documentation. When comparing our MIS and their module implementation, the resemblance between the two is evident. It was also nice to see that the module they provided us with does feel like a solid basis for the logic we were aiming to implement. I could see our team using it as a starting point to extrapolate on when looking to integrate this module into our program. 

Overall, the group that implemented our module didn’t have many questions, which is a promising indicator that documentation was clear and easy to understand. The only time they reached out was to see if there were any sections in the other documentation that we had done (outside the MIS) that would be helpful to review. We did provide them a few sections in the SRS (S2.2.3, S.4.2, \& S.4.4) as a reference in case they wanted greater detail for how the module would be used. We also took this opportunity to clarify that ultimately this module should focus on the backend, with its functionality being invoked by a separate frontend-focused module. Reflecting back, the clarity of our documentation could be improved by explicitly indicating whether each module is frontend- or backend-focused. For example, the short section 13.1 of the MIS does state that it is tracking incremental changes triggered by user modification. In retrospect, phrasing such as “triggered by user modification” may be interpreted as implying frontend involvement. Cases like this are instances where being mindful of wording within each module description would add further clarity and would likely reduce the chance of oversight. It is important to remember that even if someone is able to review and understand the entire MIS, it can still be difficult to remember all the details on how the modules interact as an outsider who doesn’t intuitively know how the program operates. 
