...
\texttt{saveProject} & id: str, data: Any & bool & IOError \\
\texttt{loadProject} & id: str & Any & FileNotFoundError \\
\texttt{deleteProject} & id: str & bool & IOError \\
\texttt{listProjects} & None & list[str] & None \\
\bottomrule
\end{tabular}
\end{center}
...
saveProject(id: str, data: Any) $\rightarrow$ bool
\begin{itemize}
  \item Transition: stores the provided serialized project data under the given project identifier in persistent storage.
  \item Output: bool indicating whether the save operation was successful.
  \item Exceptions: IOError if the data cannot be written to persistent storage.
\end{itemize}

loadProject(id: str) $\rightarrow$ Any
\begin{itemize}
  \item Transition: retrieves the serialized project data associated with the specified project identifier from persistent storage.
  \item Output: Any representing the loaded serialized project data.
  \item Exceptions: FileNotFoundError if no project exists with the given identifier.
\end{itemize}

deleteProject(id: str) $\rightarrow$ bool
\begin{itemize}
  \item Transition: removes the project data associated with the specified project identifier from persistent storage.
  \item Output: bool indicating whether the deletion was successful.
  \item Exceptions: IOError if the project data cannot be removed.
\end{itemize}

listProjects() $\rightarrow$ list[str]
\begin{itemize}
  \item Transition: retrieves a list of all project identifiers currently stored in persistent storage.
  \item Output: list[str] containing all stored project identifiers.
  \item Exceptions: None.
\end{itemize}
