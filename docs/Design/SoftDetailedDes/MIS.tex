...
\item \texttt{has\_material(voxel: Voxel) -> bool}: Checks if a voxel has an assigned material property
\item \texttt{has\_magnetization(voxel: Voxel) -> bool}: Checks if a voxel has an assigned magnetization vector
...
\item \texttt{validate\_voxel(voxel: Voxel) -> bool}: Validates that voxel has required structure and valid property values
