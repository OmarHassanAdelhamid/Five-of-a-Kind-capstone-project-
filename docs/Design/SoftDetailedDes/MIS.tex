...
\subsection{Uses}
\begin{itemize}
  \item Display Partitioning (M3) — tracks interface states across partitions.
  \item Graphics Adapter (M18) — renders updates visible to the user.
  \item Model Manager (M9) — manages model and metadata updates affecting visualization.
  \item Highlight Manager (M13) — manages voxel highlight changes.
  \item ModelStructure (M17) — defines specifications for update bundles within the model.
\end{itemize}
...
\subsection{Uses}
\begin{itemize}
  \item VisualizationStateManager (M8) — provides rendering instructions.
  \item ModelManager (M9) — supplies geometry data for visualization.
  \item BackendCommunicationManager (M6) — manages render requests to backend APIs.
\end{itemize}
