...
\begin{description}
\item[\refstepcounter{ucnum} \uctheucnum\label{ucStandalone}:] Changing from a standalone application to a plugin-based/CAD integration software. While this would bring its own set of benefits, this would require an entire system restructuring, as well as add complexity from the added requirements of complying to the external software's standards. Additionally, this would limit the number of users that could convieniently use the software.

\item[\refstepcounter{ucnum} \uctheucnum\label{ucWorkflow}:] Fundamental changes to the user's workflow (eg. transitioning from file import, to editing voxels, to exporting the project). Ultimately this software is being developed to expediate an existing specific, straightforward workflow. Not only would changes to this confuse users and slow the transition to using this tool, it would also require drastic changes to the entire system architecture. 
...
\begin{description}
\item[Secrets:] How to extract, parse, and normalize data from any compatible input file into the model’s internal format. How to identify file type, and how to parse and interpret 3D geometry.
...
\item[Type of Module:] Library: a reusable parsing and normalization component that handles all input data extraction for new projects.
\end{description}

\subsubsection{Voxel Slicing Module (\mref{m2})}

\begin{description}
\item[Secrets:] How to convert the input geometry into a set of voxels. How to slice along all axes and optimize voxelization resolution.
\item[Services:] Converts geometric mesh into voxel representation. Generates discrete voxels by slicing through the 3D model at fixed intervals. 
\item[Implemented By:] \progname{}
\item[Type of Module:] Library: a computational module responsible for transforming geometry into voxelized form.
\end{description}
...
\begin{description}
  ...
  between display segments. How to represent partition boundaries. How to minimize the number of partitions created. How to ensure that display partitions maintain balanced spatial proportions. How to validate that a partition of voxels is viable to display within the UI.
  ...
  \item[Implemented By:] \progname{}
  \item[Type of Module:] Library: a reusable set of functions responsible for defining and partitioning sets of voxels into display segments.
\end{description}

\subsubsection{Project Manager Module (\mref{m4})}

\begin{description}
\item[Secrets:] How to create and initialize a new project workspace. How to establish the intitial project structure, metadata, and default configuration settings. How to allocate and manage project resources and state. How to save and load projects and how frequently to autosave. How multiple workspaces are managed.
\item[Services:] Creates new projects with appropriate initialization. Manages loading projects, and the autonomous and manual saving of projects. Manages swapping between workspaces.
\item[Implemented By:] \progname{}
...
\begin{description}
  \item[Secrets:] How events are internally handled. How to differentiate between similar events. How to define the rules that associate events with actions to take. How to handle events where user intent is inconclusive. How to determine when multiple events should be interpreted as a single action. How to apply context from UI when interpreting events.
  \item[Services:] Maps events from interaction with UI tosystem actions that reflect user intent.
  \item[Implemented By:] \progname{}
  \item[Type of Module:] Abstract Object: a component responsible for internal event handling and action routing that internally must track the current view displayed to the user for context.
\end{description}
...
\begin{description}
\item[Secrets:] How visualization-related state is represented and maintained independently of model modification, user interaction, and persistence mechanisms. How changes to the underlying model are reflected as visualization state transitions without exposing rendering or interaction logic.
\item[Services:] Maintains the current visualization state of the system, including active views and highlight states. Coordinates visualization state updates by aggregating state change descriptors from upstream modules and forwarding structured update requests to the Graphics Adapter.
\item[Implemented By:] \progname{}
\item[Type of Module:] Abstract Object: a component responsible for maintaining visualization state and coordinating visualization updates.
\end{description}
...
\begin{description}
\item[Secrets:] How model state is represented and mutated, including voxel updates and material and magnetization assignments. How model state is persisted through explicit save operations of the model file.
\item[Services:] Manages voxel-based model state by providing operations to add, remove, and modify voxels and update material and magnetization properties. Provides explicit save operations for persisting model state. Maintains model-wide consistency and exposes model state changes to dependent modules without coordinating visualization or user interaction.
\item[Implemented By:] \progname{}
\item[Type of Module:] Abstract Object: a component responsible for model state management and persistence.
\end{description}
...
\begin{description}
  \item[Secrets:] How to locate and track voxels that satisfy particular property criteria, such as selection state, material type, or user-defined rules. How to interpret the voxel grid data structure to efficiently identify and access relevant voxels without exposing model mutation or interaction logic.
  \item[Services:] Interprets the voxel data structure to find and return voxels meeting specified property criteria (e.g., all voxels with a given material, within a spatial region, or within a given layer). Enables other modules (such as Highlight Manager) to query voxel sets based on their properties.
  \item[Implemented By:] \progname{}
  \item[Type of Module:] Library: a computational component that provides read-only querying and tracking of voxels with specific properties within the voxel grid.
\end{description}
...
\begin{description}
  ...
  \item[Implemented By:] \progname{}
  \item[Type of Module:] Abstract Object: must maintain a constant state of the mapping between highlight colour and material IDs.
\end{description}
...
\begin{description}
\item[Secrets:] How to communicate with the external graphics API to render images to the user's screen. How to ensure the graphics API is currently operational.
\item[Services:] Requests renders from the graphics API given a set of updates. Checks whether the graphics API is currently operational or not.
\item[Implemented By:] \progname{}
\item[Type of Module:] Library: A reusable component responsible for communication with an API.
\end{description}
...
\toprule
\textbf{Req.} 
& \mref{m1} & \mref{m2} & \mref{m3} & \mref{m4} 
& \mref{m7} & \mref{m8} & \mref{m9} & \mref{m10} 
& \mref{m12} & \mref{m13} & \mref{m14} & \mref{m15} 
& \mref{m16} & \mref{m17} & \mref{m18} & \mref{m20} \\
\midrule

F211 &
\xbox & \obox & \obox & \xbox & \obox & \obox & \obox & \obox &
\obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox \\
...
F213 &
\xbox & \xbox & \obox & \obox & \xbox & \obox & \obox & \obox &
\obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox \\

F214 &
\xbox & \obox & \obox & \obox & \xbox & \obox & \obox & \obox &
\obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox \\
...
\obox & \obox & \obox & \obox & \obox & \xbox & \xbox & \obox \\

F222 &
\obox & \obox & \xbox & \obox & \xbox & \xbox & \obox & \obox &
\obox & \obox & \obox & \obox & \obox & \xbox & \obox & \obox \\

F223 &
\obox & \obox & \obox & \obox & \xbox & \xbox & \obox & \obox &
\obox & \obox & \obox & \obox & \obox & \xbox & \xbox & \obox \\
...
F225 &
\obox & \obox & \obox & \obox & \xbox & \xbox & \obox & \obox &
\xbox & \xbox & \obox & \obox & \obox & \xbox & \xbox & \obox \\
...
F231 &
\obox & \obox & \obox & \obox & \xbox & \obox & \xbox & \obox &
\obox & \obox & \obox & \obox & \obox & \xbox & \obox & \obox \\
...
F235 &
\obox & \obox & \obox & \obox & \xbox & \obox & \xbox & \xbox &
\xbox & \obox & \obox & \obox & \obox & \xbox & \obox & \obox \\
...
F238 &
\obox & \obox & \obox & \obox & \xbox & \obox & \xbox & \obox &
\obox & \xbox & \obox & \obox & \obox & \xbox & \obox & \obox \\
...
F241 &
\obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox &
\xbox & \obox & \xbox & \obox & \obox & \xbox & \obox & \obox \\

F242 &
\obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox &
\obox & \obox & \obox & \xbox & \obox & \xbox & \obox & \xbox \\

F243 &
\obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox &
\obox & \obox & \obox & \xbox & \obox & \xbox & \obox & \obox \\

F244 &
\obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox &
\obox & \obox & \obox & \xbox & \xbox & \obox & \obox & \xbox \\
...
NF211 &
\xbox & \xbox & \obox & \xbox & \obox & \obox & \obox & \obox &
\obox & \obox & \obox & \obox & \xbox & \obox & \obox & \obox \\
...
NF222 &
\obox & \obox & \xbox & \obox & \obox & \obox & \obox & \obox &
\xbox & \obox & \obox & \obox & \xbox & \obox & \xbox & \obox \\
...
NF241 &
\obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox &
\obox & \obox & \xbox & \xbox & \xbox & \obox & \obox & \obox \\

NF242 &
\obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox &
\obox & \obox & \obox & \xbox & \xbox & \xbox & \obox & \xbox \\
...
\acref{acInput} &
\xbox & \obox & \obox & \obox & \obox & \obox & \obox & \obox &
\obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox \\

\acref{acInputConstraints} &
\xbox & \obox & \xbox & \obox & \obox & \obox & \obox & \obox &
\obox & \obox & \obox & \obox & \obox & \xbox & \obox & \obox \\

\acref{acVoxelSize} &
\obox & \xbox & \xbox & \obox & \obox & \obox & \obox & \obox &
\obox & \obox & \obox & \obox & \obox & \xbox & \obox & \obox \\
...
\acref{acSelection} &
\obox & \obox & \obox & \obox & \xbox & \obox & \obox & \obox &
\xbox & \obox & \obox & \obox & \obox & \obox & \xbox & \obox \\

\acref{acExport} &
\obox & \obox & \obox & \obox & \obox & \obox & \obox & \obox &
\obox & \obox & \obox & \xbox & \obox & \obox & \obox & \xbox \\
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/UsesHierarchyFINAL.pdf}
  \caption{Uses hierarchy among modules.}
  \label{FigUH}
\end{figure}

\section*{Module Assignment and Implementation Timeline}

Table~\ref{tab:module-timeline} summarizes the implementation responsibility and planned completion timeline for each module in the system. Each module identifier is hyperlinked to its corresponding detailed module specification to support traceability between the software architecture and detailed design.

% filepath: /home/abdelo8/Capstone/Five-of-a-Kind-capstone-project-/docs/Design/SoftArchitecture/MG.tex

\begin{table}[H]
\centering
\caption{Module assignment and implementation timeline}
\label{tab:module-timeline}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{|l|l|l|}
\hline
\textbf{Module} & \textbf{Assignee(s)} & \textbf{Completion Date} \\
\hline
\hyperref[m1]{M1: Input Interpreter} & Andrew, Daniel & 24 Nov 2025 \\
\hyperref[m2]{M2: Voxel Slicing} & Andrew, Daniel & 24 Nov 2025 \\
\hyperref[m3]{M3: Display Partitioning} & Omar, Olivia & 22 Jan 2026 \\
\hyperref[m4]{M4: Project Manager} & Khalid, Andrew & 24 Jan 2026 \\
\hyperref[m7]{M5: Interaction Controller} & Andrew, Daniel & 24 Nov 2025 \\
\hyperref[m8]{M6: Visualization State Manager} & Andrew, Daniel & 24 Nov 2025 \\
\hyperref[m9]{M7: Model Manager} & Andrew, Daniel & 26 Jan 2026 \\
\hyperref[m10]{M8: History Manager} & Khalid, Olivia & 27 Jan 2026 \\
\hyperref[m12]{M9: Voxel Tracking} & Omar, Daniel & 28 Jan 2026 \\
\hyperref[m13]{M10: Highlight Manager} & Omar, Olivia & 29 Jan 2026 \\
\hyperref[m14]{M11: Export Validation} & Omar, Khalid & 30 Jan 2026 \\
\hyperref[m15]{M12: Export Manager} & Khalid, Olivia & 31 Jan 2026 \\
\hyperref[m16]{M13: Error Diagnostic Handler} & Andrew, Khalid & 1 Feb 2026 \\
\hyperref[m17]{M14: Model Structure} & Omar, Daniel & 25 Jan 2026 \\
\hyperref[m18]{M15: Graphics Adapter} & Olivia, Khalid & 2 Feb 2026 \\
\hyperref[m20]{M16: Export Structure} & Andrew, Daniel & 24 Nov 2025 \\
\hline
\end{tabular}
\end{table}
