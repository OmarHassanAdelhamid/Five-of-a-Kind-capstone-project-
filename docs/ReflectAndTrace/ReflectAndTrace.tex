...
\begin{itemize}
  ...
  \item \textbf{Voxelization Algorithms:} Converting arbitrary triangle meshes to volumetric voxel representations is a specialized computational geometry topic not covered in the curriculum. The team had to research and implement techniques for ray casting, surface-to-volume conversion, and handling not smooth geometry cases independently.

  \item \textbf{3D File Format Parsing:} Parsing industry standard 3D model formats of STL (both ASCII and binary variants) required studying their specifications directly from documentation. This included understanding how face normals, vertex winding order, and mesh topology are encoded and how to handle malformed inputs correctly.
