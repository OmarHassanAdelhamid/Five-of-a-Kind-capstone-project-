...
export const setupCameraForVoxels = (
  ...
  coordinates.forEach((coord) => {
    box.expandByPoint(
      new THREE.Vector3(coord[0], coord[1], coord[2]).multiplyScalar(
        sceneScale,
      ),
    );
  });
  ...
  const sortedYs = [...new Set(coordinates.map((c) => c[1]))].sort(
    (a, b) => a - b,
  );
  let minPitch = 1;
  for (let i = 1; i < sortedYs.length; i++) {
    const d = sortedYs[i] - sortedYs[i - 1];
    if (d > 0 && d < minPitch) minPitch = d;
  }
  grid.position.y = -size.y / 2 - (minPitch * sceneScale) / 2;
};
...
export const renderVoxelInstanced = (
  ...
  if (!Number.isFinite(voxelSize) || voxelSize <= 0) {
    ...
    throw new Error(
      `Invalid voxelSize received from backend: ${String(voxelSize)}`,
    );
  }
