...
describe('UploadButton', () => {
  it('calls onUpload with file when .stl file selected', async () => {
    ...
    const file = new File(['stl'], 'model.stl', {
  ...
  it('shows Uploading… when uploading', () => {
    const { getByText } = render(
      <UploadButton uploadState="uploading" onUpload={jest.fn()} />,
    );
  ...
  it('disables input when uploading', () => {
    ...
    expect(
      (document.getElementById('stl-upload-input') as HTMLInputElement)
        .disabled,
    ).toBe(true);
  });
  ...
  it('shows Upload STL File when idle', () => {
    const { getByText } = render(
      <UploadButton uploadState="idle" onUpload={jest.fn()} />,
    );
